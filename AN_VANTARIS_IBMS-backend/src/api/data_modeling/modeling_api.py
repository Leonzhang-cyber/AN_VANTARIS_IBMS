...
@jwt_required
@require_permission('modeling:read')
def list_csv_files():
    """
    获取所有 CSV 文件列表
    返回每个设备的设备号和文件信息
...
@jwt_required
@require_permission('modeling:train')
def train(device_code):
    """训练模型"""
    log_sensitive_api("modeling.train", device_code=device_code, category="modeling")
    try:
...
@jwt_required
@require_permission('modeling:predict')
def predict(device_code):
    """单点预测"""
    log_sensitive_api("modeling.predict", device_code=device_code, category="modeling")
    try:
...
def predict_future(device_code):
    ...
    """
    log_sensitive_api("modeling.predict_future", device_code=device_code, category="modeling")
    try:
...
@jwt_required
@require_any_permission(['modeling:read', 'modeling:predict'])
