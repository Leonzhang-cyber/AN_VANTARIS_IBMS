"""Read-only Excel workbook access for synthetic evidence intake."""
...
from .constants import BUSINESS_SHEETS, REQUIRED_SHEETS
...
class ExcelWorkbook:
    """Minimal read-only .xlsx accessor using the standard library only."""
    ...
    def validate_extension(path: Path) -> None:
        ...
        if suffix in {".xls", ".xlsm", ".xlsb"}:
            raise ExcelIntakeError("UNSUPPORTED_EXTENSION", f"unsupported workbook extension: {suffix}")
        raise ExcelIntakeError("UNSUPPORTED_EXTENSION", "only .xlsx workbooks are accepted")

    @staticmethod
    def open(path: Path) -> "ExcelWorkbook":
        resolved = path.resolve()
        ExcelWorkbook.validate_extension(resolved)
        if not resolved.is_file():
            raise ExcelIntakeError("WORKBOOK_NOT_FOUND", "workbook file was not found")
        try:
            book = ExcelWorkbook(resolved)
        except zipfile.BadZipFile as exc:
            raise ExcelIntakeError("INVALID_WORKBOOK", "workbook is not a valid xlsx archive") from exc
        if book.has_macros():
            book.close()
            raise ExcelIntakeError("MACROS_NOT_ALLOWED", "macro-enabled workbook content is not accepted")
        if book.is_password_protected():
            book.close()
            raise ExcelIntakeError("PASSWORD_PROTECTED", "password-protected workbooks are not accepted")
        if book.has_external_links():
            book.close()
            raise ExcelIntakeError("EXTERNAL_LINKS", "external workbook links are not accepted")
        missing = [name for name in REQUIRED_SHEETS if name not in book.sheet_names()]
        if missing:
            book.close()
            raise ExcelIntakeError("MISSING_WORKSHEET", f"required worksheets missing: {', '.join(missing)}")
        return book
    ...
    def rows(self, sheet_name: str) -> list[dict[str, Any]]:
        if sheet_name not in self._sheet_map:
            raise ExcelIntakeError("MISSING_WORKSHEET", f"worksheet not found: {sheet_name}")
        ...
        if sheet_name in BUSINESS_SHEETS and formulas:
            raise ExcelIntakeError("FORMULAS_NOT_ALLOWED", "formulas are not allowed in business worksheets")
        if not table:
            return []
        header_row = table[min(table)]
        headers = [_stringify(header_row.get(idx, "")).strip() for idx in sorted(header_row)]
        if any(not header for header in headers):
            raise ExcelIntakeError("EMPTY_HEADER", f"worksheet {sheet_name} contains empty header cells")
        if len(headers) != len(set(headers)):
            raise ExcelIntakeError("DUPLICATE_HEADERS", f"worksheet {sheet_name} contains duplicate headers")
        records: list[dict[str, Any]] = []
        for row_index in sorted(index for index in table if index > min(table)):
