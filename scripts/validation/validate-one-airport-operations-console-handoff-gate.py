#!/usr/bin/env python3
"""Validate A5-02 Airport Operations Console Package readiness and handoff gate."""
...
REGISTRY = ROOT / "AN_VANTARIS_ONE/registries/operations-console-handoff-gate.v1.json"
GENERIC_DIR = ROOT / "AN_VANTARIS_ONE/operations_console_handoff_gate"
AIRPORT_MODULE = ROOT / "AN_VANTARIS_ONE/industry_profiles/airport/operations_console_handoff_gate.py"
AIRPORT_PROJECTION = ROOT / "AN_VANTARIS_ONE/industry_profiles/airport/projections/airport-operations-console-handoff-gate.v1.json"
RUNNER = ROOT / "scripts/validation/_run_a5_02_airport_operations_console_handoff_gate.py"

ALLOWED_PREFIXES = (
    "AN_VANTARIS_ONE/operations_console_handoff_gate/",
    "AN_VANTARIS_ONE/registries/operations-console-handoff-gate.v1.json",
    "AN_VANTARIS_ONE/industry_profiles/airport/operations_console_handoff_gate.py",
    "AN_VANTARIS_ONE/industry_profiles/airport/projections/airport-operations-console-handoff-gate.v1.json",
    "AN_VANTARIS_ONE/tests/operations_console_handoff_gate/",
    "scripts/validation/validate-one-airport-operations-console-handoff-gate.py",
    "scripts/validation/_run_a5_02_airport_operations_console_handoff_gate.py",
    "ONE_AIRPORT_A5_02_AIRPORT_CONSOLE_PACKAGE_READINESS_AND_HANDOFF_GATE_REPORT.md",
)
...
def _append_projection_checks(checks: list[tuple[str, bool]]) -> None:
    ...
    api = gate.get("apiReadinessContract", {})
    frontend = gate.get("frontendReadinessContract", {})
    decision = gate.get("handoffDecision", {})
    checks.append(("Implementation status frozen", gate.get("implementationStatus") == "AIRPORT_CONSOLE_PACKAGE_HANDOFF_GATE_COMPLETE"))
    checks.append(("Readiness outcome frozen", gate.get("readinessOutcome") == "AIRPORT_CONSOLE_PACKAGE_READY_FOR_API_FRONTEND_PLANNING"))
    checks.append(("Expected summary freeze", all(summary.get(key) == value for key, value in EXPECTED_SUMMARY.items())))
    checks.append(("Eight page handoff entries", len(gate.get("pageHandoffMatrix", [])) == 8))
    checks.append(("Fifteen datasource handoff entries", len(gate.get("dataSourceHandoffMatrix", [])) == 15))
    checks.append(("Eight card handoff entries", len(gate.get("cardHandoffMatrix", [])) == 8))
    checks.append(("Fifteen release gates", len(gate.get("releaseGateResults", [])) == 15))
    checks.append(("All release gates pass", all(item.get("status") == "PASS" for item in gate.get("releaseGateResults", []))))
    checks.append(("All page entries unblocked", all(item.get("blocked") is False for item in gate.get("pageHandoffMatrix", []))))
    checks.append(("All data sources read-only", all(source.get("readOnly") is True for source in gate.get("dataSourceHandoffMatrix", []))))
    checks.append(("All data source boundaries disabled", all(source.get("apiEnabled") is False and source.get("frontendEnabled") is False and source.get("databaseAccessEnabled") is False for source in gate.get("dataSourceHandoffMatrix", []))))
    checks.append(("API readiness contract candidate", api.get("contractState") == "READY_FOR_HANDOFF" and api.get("candidateEndpointCount") == 8 and api.get("readOnlyEndpointCandidateCount") == 8 and api.get("endpointImplementationAllowed") is False and api.get("publicApiEnabled") is False and api.get("databaseAccessAllowed") is False and api.get("writeOperationAllowed") is False))
    checks.append(("Frontend readiness contract candidate", frontend.get("contractState") == "READY_FOR_HANDOFF" and frontend.get("pageCandidateCount") == 8 and frontend.get("cardCandidateCount") == 8 and frontend.get("frontendImplementationAllowed") is False and frontend.get("routeImplementationAllowed") is False and frontend.get("runtimeDataMutationAllowed") is False))
    checks.append(("Handoff decision", decision.get("decisionState") == "READY_FOR_HANDOFF" and decision.get("handoffAllowed") is True and decision.get("apiImplementationAllowed") is False and decision.get("frontendImplementationAllowed") is False and decision.get("databaseWriteAllowed") is False and decision.get("runtimeActivationAllowed") is False and decision.get("productionActivationAllowed") is False and decision.get("pushAllowed") is False))
...
def main() -> int:
    ...
    checks.append(("Airport handoff gate module exists", AIRPORT_MODULE.is_file()))
    checks.append(("Airport handoff gate artifact exists", AIRPORT_PROJECTION.is_file()))
    ...
    if REGISTRY.is_file():
        ...
        checks.append(("Registry handoff allowed", registry.get("handoffAllowed") is True))
    ...
    checks.append(("Changes within A5-02 allowed paths", len(disallowed) == 0))
    if disallowed:
    ...
    test_proc = _run(["python3", "-m", "unittest", "discover", "-s", "AN_VANTARIS_ONE/tests/operations_console_handoff_gate", "-p", "test_*.py"], env=env)
    checks.append(("Focused A5-02 tests pass", test_proc.returncode == 0))
    if test_proc.returncode != 0:
        errors.append(test_proc.stdout[-4000:] or test_proc.stderr[-4000:] or "focused A5-02 tests failed")
    ...
    if AIRPORT_PROJECTION.is_file():
        ...
        if second.returncode != 0 or before != after:
            errors.append("operations console handoff gate generation is not byte-identical")

    print("ONE_AIRPORT_A5_02_AIRPORT_CONSOLE_PACKAGE_READINESS_AND_HANDOFF_GATE_VALIDATION")
