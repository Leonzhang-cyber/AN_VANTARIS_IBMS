#!/usr/bin/env python3
"""Validate UMMS-R9A local freeze and optional tag plan."""
...
FREEZE_DOC = ROOT / "ONE_UMMS_R9A_LOCAL_FREEZE_AND_OPTIONAL_TAG_PLAN.md"
REGISTRY = ROOT / "AN_VANTARIS_ONE/registries/umms-r9a-local-freeze.v1.json"
REPORT = ROOT / "ONE_UMMS_R9A_LOCAL_FREEZE_AND_OPTIONAL_TAG_PLAN_REPORT.md"
UMMS_R9_PROJECTION = ROOT / "AN_VANTARIS_ONE/projections/umms-airport-hmi-locator-binding-readiness.v1.json"
UMMS_R9_REGISTRY = ROOT / "AN_VANTARIS_ONE/registries/umms-airport-hmi-locator-binding-readiness-registry.v1.json"
AIRPORT_CLIENT = ROOT / "AN_VANTARIS_IBMS-frontend/src/services/api/airportGaReadonly.ts"
PASS_MARKER = "ONE_UMMS_R9A_LOCAL_FREEZE_AND_OPTIONAL_TAG_PLAN_PASS"

UMMS_R9_COMMIT = "85230dc864d9ff2fbd2d9ddb45bf50891e4a2f5f"
UMMS_R9_MARKER = "ONE_UMMS_R9_AIRPORT_HMI_LOCATOR_BINDING_READINESS_PASS"
FREEZE_SCOPE = (
    "UMMS HMI locator binding readiness model",
    "Work Order HMI locator binding model",
    "Fault Case HMI locator binding model",
    "PM HMI locator binding model",
    "Inventory HMI locator binding model",
    "Vendor / Contract / SLA HMI locator binding model",
    "UCDE evidence HMI context model",
    "Locator reference validation model",
    "HMI locator validation gate model",
    "Airport HMI readiness dependency map",
    "Customer core function R9 alignment",
...
def _changed_paths() -> set[str]:
    ...
    if _run(["git", "log", "-1", "--pretty=%s"]).stdout.strip() == "docs(one): freeze umms airport hmi locator binding readiness":
...
def main() -> int:
    ...
    checks.append(("UMMS-R9A freeze document exists", FREEZE_DOC.is_file()))
    checks.append(("Optional registry exists", REGISTRY.is_file()))
    checks.append(("UMMS-R9A report exists", REPORT.is_file()))
    checks.append(("UMMS-R9 commit reference is present", UMMS_R9_COMMIT in freeze_text and UMMS_R9_COMMIT in registry_text and UMMS_R9_COMMIT in report_text))
    checks.append(("UMMS-R9 PASS marker is referenced", UMMS_R9_MARKER in combined))
    checks.append(("Freeze scope includes all required UMMS-R9 scope items", all(item in freeze_text for item in FREEZE_SCOPE)))
    checks.append(("UMMS-R9 summary states UMMS is generic and not Airport-specific", "UMMS remains a generic cross-industry maintenance management module." in freeze_text and "not UMMS core identity" in freeze_text and "not Airport-specific logic" in freeze_text))
    checks.append(("UMMS-R9 summary states no HMI runtime, drawing upload, BIM runtime, topology runtime, equipment control, device connection, or connector execution exists", "No HMI runtime, drawing upload, BIM runtime, topology runtime, equipment control, device connection, or connector execution exists." in freeze_text))
    ...
    checks.append(("No HMI runtime/control execution claim", "This is not HMI runtime execution." in freeze_text and "This is not HMI control execution." in freeze_text and "hmi runtime execution enabled" not in lower_combined and "hmi control execution enabled" not in lower_combined))
    checks.append(("No drawing upload claim", "This is not drawing upload." in freeze_text and "drawing upload enabled" not in lower_combined))
    checks.append(("No BIM/topology runtime claim", "This is not BIM runtime integration." in freeze_text and "This is not topology runtime integration." in freeze_text and "bim runtime integration enabled" not in lower_combined and "topology runtime integration enabled" not in lower_combined))
    checks.append(("No equipment control claim", "This is not equipment control." in freeze_text and "equipment control enabled" not in lower_combined))
    checks.append(("No device connection claim", "This is not device connection." in freeze_text and "device connection enabled" not in lower_combined))
    checks.append(("No work order/PM/inventory/vendor/evidence runtime claim", "This is not work order runtime execution." in freeze_text and "This is not PM execution." in freeze_text and "This is not inventory transaction." in freeze_text and "This is not vendor / contract / SLA execution." in freeze_text and "This is not evidence upload or closure execution." in freeze_text))
    ...
    checks.append(("UMMS-R9A PASS marker present", PASS_MARKER in combined))
    ...
    checks.append(("Changes limited to UMMS-R9A freeze scope", not disallowed))
    ...
    if disallowed:
        errors.append(f"changes outside UMMS-R9A scope: {', '.join(sorted(disallowed))}")
    if forbidden:
    ...
    checks.append(("Registry freeze id present", registry_data.get("releaseFreezeId") == "umms-r9a-local-freeze-20260621"))
    ...
    checks.append(("Registry safety matrix frozen", safety.get("readOnly") is True and safety.get("oneAdapterIntroduced") is False and safety.get("hmiRuntimeExecution") is False and safety.get("drawingUpload") is False and safety.get("workOrderRuntimeExecution") is False))
    checks.append(("Registry validation matrix includes UMMS-R9 marker", UMMS_R9_MARKER in registry_data.get("validationMatrix", {})))
    ...
    checks.append(("Projection JSON validation passes", _json_valid(UMMS_R9_PROJECTION)))
    checks.append(("Registry JSON validation passes", _json_valid(UMMS_R9_REGISTRY) and _json_valid(REGISTRY)))

    print("ONE_UMMS_R9A_LOCAL_FREEZE_VALIDATION")
    failed = False
