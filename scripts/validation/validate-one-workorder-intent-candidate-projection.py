#!/usr/bin/env python3
"""Validate A3-04 WorkOrderIntent candidate projection."""
...
REGISTRY = ROOT / "AN_VANTARIS_ONE/registries/workorder-intent-candidate-projection.v1.json"
GENERIC_DIR = ROOT / "AN_VANTARIS_ONE/workorder_intent_candidate"
AIRPORT_MODULE = ROOT / "AN_VANTARIS_ONE/industry_profiles/airport/workorder_intent_candidate_projection.py"
AIRPORT_PROJECTION = ROOT / "AN_VANTARIS_ONE/industry_profiles/airport/projections/airport-workorder-intent-candidates.v1.json"
RUNNER = ROOT / "scripts/validation/_run_a3_04_workorder_intent_candidate_projection.py"

ALLOWED_PREFIXES = (
    "AN_VANTARIS_ONE/workorder_intent_candidate/",
    "AN_VANTARIS_ONE/registries/workorder-intent-candidate-projection.v1.json",
    "AN_VANTARIS_ONE/industry_profiles/airport/workorder_intent_candidate_projection.py",
    "AN_VANTARIS_ONE/industry_profiles/airport/projections/airport-workorder-intent-candidates.v1.json",
    "AN_VANTARIS_ONE/tests/workorder_intent_candidate/",
    "scripts/validation/validate-one-workorder-intent-candidate-projection.py",
    "scripts/validation/_run_a3_04_workorder_intent_candidate_projection.py",
    "ONE_AIRPORT_A3_04_WORKORDER_INTENT_CANDIDATE_PROJECTION_REPORT.md",
)
FORBIDDEN_PATHS = (
    "AN_VANTARIS_EDGE/",
    "AN_VANTARIS_LINK/",
...
def _append_projection_checks(checks: list[tuple[str, bool]]) -> None:
    ...
    checks.append(("Implementation status frozen", projection.get("implementationStatus") == "WORKORDER_INTENT_CANDIDATE_PROJECTION_COMPLETE"))
    checks.append(("Readiness outcome frozen", projection.get("readinessOutcome") == "WORKORDER_INTENT_CANDIDATES_COMPLETE_WITH_PENDING_REVIEWS"))
    checks.append(("Five candidates", len(candidates) == 5 and summary.get("workOrderIntentCandidateCount") == 5))
    checks.append(("Ten review cards", len(cards) == 10 and summary.get("reviewCardCount") == 10))
    ...
    checks.append(("Expected intent type counts", summary.get("correctiveMaintenanceCandidateCount") == 2 and summary.get("investigationRequestCandidateCount") == 3))
    checks.append(("Expected disciplines", {key: by_key.get(key, {}).get("proposedTradeDiscipline") for key in ("ACS", "RAS", "CCTV", "PA", "TEL")} == {"ACS": "SECURITY", "RAS": "COMMUNICATION", "CCTV": "SECURITY", "PA": "ELV", "TEL": "COMMUNICATION"}))
    ...
    checks.append(("Blocked by FaultCase review", summary.get("blockedByFaultCaseReviewCount") == 5))
    ...
    checks.append(("Work policy review required", summary.get("workManagementPolicyReviewRequiredCount") == 5))
    ...
    for key in ("workOrderIntentCreatedCount", "workOrderCreatedCount", "ummsWriteCount", "oneWorkManagementWriteCount", "canonicalWriteCount", "databaseWriteCount", "runtimeAlarmObservedCount"):
...
def main() -> int:
    ...
    checks.append(("Changes within A3-04 allowed paths", len(disallowed) == 0))
    if disallowed:
    ...
    test_proc = _run(["python3", "-m", "unittest", "discover", "-s", "AN_VANTARIS_ONE/tests/workorder_intent_candidate", "-p", "test_*.py"], env={**os.environ, "PYTHONPATH": str(ROOT / "AN_VANTARIS_ONE")})
    checks.append(("Focused A3-04 tests pass", test_proc.returncode == 0))
    if test_proc.returncode != 0:
        errors.append(test_proc.stdout[-4000:] or test_proc.stderr[-4000:] or "focused A3-04 tests failed")
    ...
    print("ONE_AIRPORT_A3_04_WORKORDER_INTENT_CANDIDATE_PROJECTION_VALIDATION")
    ...
    if errors:
        ...
        return 1
    print("WORKORDER_INTENT_CANDIDATE_PROJECTION_COMPLETE")
    print("WORKORDER_INTENT_CANDIDATES_COMPLETE_WITH_PENDING_REVIEWS")
    print("ONE_AIRPORT_A3_04_WORKORDER_INTENT_CANDIDATE_PROJECTION_PASS")
    return 0
