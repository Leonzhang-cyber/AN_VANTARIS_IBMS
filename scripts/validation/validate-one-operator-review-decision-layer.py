#!/usr/bin/env python3
"""Validate A4-01 Operator Review Decision Layer."""
...
REGISTRY = ROOT / "AN_VANTARIS_ONE/registries/operator-review-decision-layer.v1.json"
GENERIC_DIR = ROOT / "AN_VANTARIS_ONE/operator_review_decision"
AIRPORT_MODULE = ROOT / "AN_VANTARIS_ONE/industry_profiles/airport/operator_review_decision_projection.py"
AIRPORT_PROJECTION = ROOT / "AN_VANTARIS_ONE/industry_profiles/airport/projections/airport-operator-review-decisions.v1.json"
RUNNER = ROOT / "scripts/validation/_run_a4_01_operator_review_decision_layer.py"
...
ALLOWED_PREFIXES = (
    ...
    "AN_VANTARIS_ONE/tests/operator_review_decision/",
...
def _append_projection_checks(checks: list[tuple[str, bool]]) -> None:
    ...
    items = projection.get("decisionItems", [])
    groups = projection.get("decisionGroups", [])
    queues = projection.get("decisionQueues", [])
    checks.append(("Implementation status frozen", projection.get("implementationStatus") == "OPERATOR_REVIEW_DECISION_LAYER_FOUNDATION_COMPLETE"))
    checks.append(("Readiness outcome frozen", projection.get("readinessOutcome") == "OPERATOR_REVIEW_DECISION_QUEUE_COMPLETE_PENDING_OPERATOR_ACTION"))
    checks.append(("Expected summary freeze", all(summary.get(key) == value for key, value in EXPECTED_SUMMARY.items())))
    checks.append(("Forty-six decision items", len(items) == 46))
    checks.append(("Eight decision groups", len(groups) == 8))
    checks.append(("Eight decision queues", len(queues) == 8))
    checks.append(("No applied decisions", all(item.get("decisionState") in {"PENDING", "READY_FOR_REVIEW"} for item in items)))
    checks.append(("No approvals", all(item.get("decisionState") != "APPROVED" for item in items)))
    ...
    checks.append(("Deterministic item ordering", items == sorted(items, key=lambda item: (item["decisionType"], item.get("sourceSystemKey") or "", item["decisionItemId"]))))
...
def main() -> int:
    ...
    checks.append(("Changes within A4-01 allowed paths", len(disallowed) == 0))
    if disallowed:
    ...
    test_proc = _run(["python3", "-m", "unittest", "discover", "-s", "AN_VANTARIS_ONE/tests/operator_review_decision", "-p", "test_*.py"], env=env)
    checks.append(("Focused A4-01 tests pass", test_proc.returncode == 0))
    if test_proc.returncode != 0:
        errors.append(test_proc.stdout[-4000:] or test_proc.stderr[-4000:] or "focused A4-01 tests failed")
    ...
    print("ONE_AIRPORT_A4_01_OPERATOR_REVIEW_DECISION_LAYER_FOUNDATION_VALIDATION")
