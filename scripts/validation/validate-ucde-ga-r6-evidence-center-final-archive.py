#!/usr/bin/env python3
"""Validate UCDE-GA-R6 Evidence Center final archive freeze."""
...
PASS_MARKER = "UCDE_GA_R6_EVIDENCE_CENTER_FINAL_ARCHIVE_PASS"

DOCS = [
    ROOT / "UCDE_GA_R6_EVIDENCE_CENTER_FINAL_ARCHIVE_SUMMARY.md",
    ROOT / "UCDE_GA_R6_BRANCH_CLOSURE_SUMMARY.md",
    ROOT / "UCDE_GA_R6_FINAL_RELEASE_CHAIN_INDEX.md",
    ROOT / "UCDE_GA_R6_FINAL_EVIDENCE_INDEX.md",
    ROOT / "UCDE_GA_R6_NEXT_PHASE_RECOMMENDATION.md",
    ROOT / "UCDE_GA_R6_REPORT.md",
]

REGISTRY = ROOT / "AN_VANTARIS_ONE/registries/ucde-ga-r6/ucde-ga-r6-evidence-center-final-archive.v1.json"

EVIDENCE_FILES = [
    ROOT / "AN_VANTARIS_ONE/registries/ucde-ga-r6/ucde-r6-final-release-chain.txt",
    ROOT / "AN_VANTARIS_ONE/registries/ucde-ga-r6/ucde-r6-final-evidence-index.txt",
    ROOT / "AN_VANTARIS_ONE/registries/ucde-ga-r6/ucde-r6-branch-closure.txt",
    ROOT / "AN_VANTARIS_ONE/registries/ucde-ga-r6/ucde-r6-next-phase-recommendation.txt",
    ROOT / "AN_VANTARIS_ONE/registries/ucde-ga-r6/ucde-r6-validator-results.txt",
]

PRIOR_REGISTRIES = [
    ROOT / "AN_VANTARIS_ONE/registries/ucde-ga-r4/ucde-ga-r4-evidence-center-customer-preview.v1.json",
    ROOT / "AN_VANTARIS_ONE/registries/ucde-ga-r5/ucde-ga-r5-evidence-center-release-index-decision.v1.json",
...
PRIOR_MARKERS = [
    ...
    "UCDE_GA_R5_EVIDENCE_CENTER_RELEASE_INDEX_DECISION_PASS",
...
def main() -> int:
    for path in DOCS + EVIDENCE_FILES + [REGISTRY]:
        if not path.exists():
            fail(f"required R6 file exists: {path.relative_to(ROOT)}")
    ok("all required R6 documents, evidence files, and registry exist")
    ...
    if registry.get("evidenceCenterFinalArchive") is not True:
        fail("registry evidenceCenterFinalArchive must be true")
    if registry.get("branchClosureSummary") is not True:
        fail("registry branchClosureSummary must be true")
    ok("registry core fields, archive flags, GO decision, and production exclusions are correct")
    ...
    for token in (
        ...
        "UCDE-GA-R5",
        "735d999de3ec158ae9f087d65a1f7d33403ff559",
        "ucde-ga-r5-evidence-center-release-decision-freeze-20260622",
    ):
        if token not in ucde_chain_text:
            fail(f"relatedUcdeReleaseChain missing {token}")
    ok("related UCDE release chain includes UCDE-GA-R4 and UCDE-GA-R5 commits/tags")
    ...
    for path in DOCS + [REGISTRY]:
        if PASS_MARKER not in read(path):
            fail(f"R6 PASS marker missing from {path.relative_to(ROOT)}")
    for phrase in DOC_PHRASES:
        if phrase not in combined_text:
            fail(f"required R6 phrase missing: {phrase}")
    ok("R6 docs contain required archive, closure, planning, and exclusion phrases")
