#!/usr/bin/env python3
"""Validate A8-03 read-only frontend implementation release gate."""
...
REGISTRY = ROOT / "AN_VANTARIS_ONE/registries/read-only-frontend-implementation-release-gate.v1.json"
GENERIC_DIR = ROOT / "AN_VANTARIS_ONE/read_only_frontend_release_gate"
AIRPORT_MODULE = ROOT / "AN_VANTARIS_ONE/industry_profiles/airport/read_only_frontend_release_gate.py"
AIRPORT_PROJECTION = ROOT / "AN_VANTARIS_ONE/industry_profiles/airport/projections/airport-read-only-frontend-implementation-release-gate.v1.json"
RUNNER = ROOT / "scripts/validation/_run_a8_03_airport_read_only_frontend_release_gate.py"

ALLOWED_PREFIXES = (
    "AN_VANTARIS_ONE/read_only_frontend_release_gate/",
    "AN_VANTARIS_ONE/registries/read-only-frontend-implementation-release-gate.v1.json",
    "AN_VANTARIS_ONE/industry_profiles/airport/read_only_frontend_release_gate.py",
    "AN_VANTARIS_ONE/industry_profiles/airport/projections/airport-read-only-frontend-implementation-release-gate.v1.json",
    "AN_VANTARIS_ONE/tests/read_only_frontend_release_gate/",
    "scripts/validation/validate-one-airport-read-only-frontend-release-gate.py",
    "scripts/validation/_run_a8_03_airport_read_only_frontend_release_gate.py",
    "ONE_AIRPORT_A8_03_READ_ONLY_FRONTEND_IMPLEMENTATION_RELEASE_GATE_REPORT.md",
)
...
EXPECTED_SUMMARY = {
    ...
    "productionRouteEnabledCount": 0, "realFrontendFileChangeCount": 0, "realMenuEntryChangeCount": 0, "liveApiCallEnabledCount": 0,
    "browserLaunchRequiredCount": 0, "networkCallRequiredCount": 0, "localhostCallRequiredCount": 0, "mutationAllowedInteractionCount": 0,
    "readOnlyFrontendImplementationAllowed": True, "productionFrontendAllowed": False, "realRouteImplementationAllowed": False,
    "menuImplementationAllowed": False, "liveApiCallAllowed": False, "dataMutationAllowed": False, "browserSmokeAllowed": False,
    "databaseWriteAllowed": False, "runtimeActivationAllowed": False, "productionActivationAllowed": False, "apiImplementationRequired": False,
    "pushAllowed": False, "apiEnabled": False, "frontendEnabled": False, "databaseWriteCount": 0, "canonicalWriteCount": 0,
    "decisionWriteCount": 0, "approvalWriteCount": 0, "auditWriteCount": 0, "containsCustomerAssetIdentifiers": False,
...
def _append_projection_checks(checks: list[tuple[str, bool]]) -> None:
    ...
    checks.append(("Implementation status frozen", gate.get("implementationStatus") == "READ_ONLY_FRONTEND_IMPLEMENTATION_RELEASE_GATE_COMPLETE"))
    checks.append(("Readiness outcome frozen", gate.get("readinessOutcome") == "READ_ONLY_FRONTEND_READY_FOR_FUTURE_UI_IMPLEMENTATION"))
    checks.append(("Expected summary freeze", all(summary.get(key) == value for key, value in EXPECTED_SUMMARY.items())))
    checks.append(("Two A8 stages", len(gate.get("stageResults", [])) == 2))
    checks.append(("All A8 stages pass", all(item.get("status") == "PASS" for item in gate.get("stageResults", []))))
    checks.append(("Eight page coverage entries", len(gate.get("pageCoverageMatrix", [])) == 8))
    checks.append(("Eight component coverage entries", len(gate.get("componentCoverageMatrix", [])) == 8))
    checks.append(("Eight interaction coverage entries", len(gate.get("interactionCoverageMatrix", [])) == 8))
    checks.append(("Seventeen release gates", len(gate.get("releaseGateResults", [])) == 17))
    ...
    checks.append(("Read-only frontend implementation decision", decision.get("decisionState") == "READY_FOR_READ_ONLY_FRONTEND_IMPLEMENTATION" and decision.get("readOnlyFrontendImplementationAllowed") is True))
    checks.append(("Production/runtime/browser/API implementation blocked", all(decision.get(key) is False for key in ("productionFrontendAllowed", "realRouteImplementationAllowed", "menuImplementationAllowed", "liveApiCallAllowed", "dataMutationAllowed", "browserSmokeAllowed", "databaseWriteAllowed", "runtimeActivationAllowed", "productionActivationAllowed", "apiImplementationRequired", "pushAllowed"))))
...
def main() -> int:
    ...
    if REGISTRY.is_file():
        ...
        checks.append(("Registry read-only implementation allowed", registry.get("readOnlyFrontendImplementationAllowed") is True))
    ...
    checks.append(("Changes within A8-03 allowed paths", len(disallowed) == 0))
    if disallowed:
    ...
    test_proc = _run(["python3", "-m", "unittest", "discover", "-s", "AN_VANTARIS_ONE/tests/read_only_frontend_release_gate", "-p", "test_*.py"], env=env)
    checks.append(("Focused A8-03 tests pass", test_proc.returncode == 0))
    if test_proc.returncode != 0:
        errors.append(test_proc.stdout[-4000:] or test_proc.stderr[-4000:] or "focused A8-03 tests failed")
    ...
    if AIRPORT_PROJECTION.is_file():
        ...
        if second.returncode != 0 or before != after:
            errors.append("read-only frontend release gate generation is not byte-identical")

    print("ONE_AIRPORT_A8_03_READ_ONLY_FRONTEND_IMPLEMENTATION_RELEASE_GATE_VALIDATION")
