#!/usr/bin/env python3
"""Validate UMMS-R4A local freeze and optional tag plan."""
...
FREEZE_DOC = ROOT / "ONE_UMMS_R4A_LOCAL_FREEZE_AND_OPTIONAL_TAG_PLAN.md"
REGISTRY = ROOT / "AN_VANTARIS_ONE/registries/umms-r4a-local-freeze.v1.json"
REPORT = ROOT / "ONE_UMMS_R4A_LOCAL_FREEZE_AND_OPTIONAL_TAG_PLAN_REPORT.md"
AIRPORT_CLIENT = ROOT / "AN_VANTARIS_IBMS-frontend/src/services/api/airportGaReadonly.ts"
UMMS_PROJECTION = ROOT / "AN_VANTARIS_ONE/projections/umms-work-order-lifecycle-state-validation-gate.v1.json"
UMMS_REGISTRY = ROOT / "AN_VANTARIS_ONE/registries/umms-work-order-lifecycle-state-validation-gate-registry.v1.json"
PASS_MARKER = "ONE_UMMS_R4A_LOCAL_FREEZE_AND_OPTIONAL_TAG_PLAN_PASS"

UMMS_R4_COMMIT = "174f81243c246b2258d47be7ccc8dbb9a3df9d5e"
UMMS_R4_MARKER = "ONE_UMMS_R4_WORK_ORDER_LIFECYCLE_STATE_VALIDATION_GATE_PASS"
FREEZE_SCOPE = (
    "Work Order lifecycle state model",
    "Transition validation model",
    "Validation gate model",
    "Role gate model",
    "Evidence gate model",
    "SLA gate model",
    "Airport-to-lifecycle mapping",
    "Customer core function R4 alignment",
...
ALLOWED_CHANGED_PREFIXES = (
    "ONE_UMMS_R4A_LOCAL_FREEZE_AND_OPTIONAL_TAG_PLAN.md",
    "AN_VANTARIS_ONE/registries/umms-r4a-local-freeze.v1.json",
    "ONE_UMMS_R4A_LOCAL_FREEZE_AND_OPTIONAL_TAG_PLAN_REPORT.md",
    "scripts/validation/validate-one-umms-r4a-local-freeze.py",
...
def _changed_paths() -> set[str]:
    ...
    if last_subject == "docs(one): freeze umms work order lifecycle gate":
...
def main() -> int:
    ...
    checks.append(("UMMS-R4A freeze document exists", FREEZE_DOC.is_file()))
    checks.append(("Optional registry exists", REGISTRY.is_file()))
    checks.append(("UMMS-R4A report exists", REPORT.is_file()))
    checks.append(("UMMS-R4 commit reference is present", UMMS_R4_COMMIT in freeze_text and UMMS_R4_COMMIT in registry_text))
    checks.append(("UMMS-R4 PASS marker is referenced", UMMS_R4_MARKER in combined))
    checks.append(("Freeze scope includes all required UMMS-R4 scope items", all(item in freeze_text for item in FREEZE_SCOPE)))
    checks.append(("UMMS-R4 summary states UMMS is generic and not Airport-specific", "UMMS remains a generic cross-industry maintenance management module." in freeze_text and "not UMMS core identity" in freeze_text and "not Airport-specific logic" in freeze_text))
    checks.append(("UMMS-R4 summary states all state transitions remain disabled", "All state transitions remain disabled." in freeze_text))
    checks.append(("UMMS-R4 summary states no state transition/workflow/approval/work order lifecycle runtime exists", "No state transition, workflow execution, approval execution, or work order lifecycle runtime exists." in freeze_text))
    ...
    checks.append(("UMMS-R4A PASS marker present", PASS_MARKER in combined))
    ...
    checks.append(("Changes limited to UMMS-R4A freeze scope", not disallowed))
    ...
    if disallowed:
        errors.append(f"changes outside UMMS-R4A scope: {', '.join(sorted(disallowed))}")
    if forbidden:
    ...
    checks.append(("Registry freeze id present", registry_data.get("releaseFreezeId") == "umms-r4a-local-freeze-20260621"))
    ...
    checks.append(("Registry validation matrix includes UMMS-R4 marker", UMMS_R4_MARKER in registry_data.get("validationMatrix", {})))
    ...
    print("ONE_UMMS_R4A_LOCAL_FREEZE_VALIDATION")
    for label, ok in checks:
        print(f"[{'PASS' if ok else 'FAIL'}] {label}")
    if errors or not all(ok for _, ok in checks):
        for error in errors:
            print(f"ERROR: {error}", file=sys.stderr)
        print("ONE_UMMS_R4A_LOCAL_FREEZE_AND_OPTIONAL_TAG_PLAN_FAIL")
        return 1
    print(PASS_MARKER)
    return 0
