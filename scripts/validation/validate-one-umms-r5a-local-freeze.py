#!/usr/bin/env python3
"""Validate UMMS-R5A local freeze and optional tag plan."""
...
FREEZE_DOC = ROOT / "ONE_UMMS_R5A_LOCAL_FREEZE_AND_OPTIONAL_TAG_PLAN.md"
REGISTRY = ROOT / "AN_VANTARIS_ONE/registries/umms-r5a-local-freeze.v1.json"
REPORT = ROOT / "ONE_UMMS_R5A_LOCAL_FREEZE_AND_OPTIONAL_TAG_PLAN_REPORT.md"
UMMS_R5_PROJECTION = ROOT / "AN_VANTARIS_ONE/projections/umms-preventive-maintenance-schedule-readiness.v1.json"
UMMS_R5_REGISTRY = ROOT / "AN_VANTARIS_ONE/registries/umms-preventive-maintenance-schedule-readiness-registry.v1.json"
AIRPORT_CLIENT = ROOT / "AN_VANTARIS_IBMS-frontend/src/services/api/airportGaReadonly.ts"
PASS_MARKER = "ONE_UMMS_R5A_LOCAL_FREEZE_AND_OPTIONAL_TAG_PLAN_PASS"

UMMS_R5_COMMIT = "89f367305d468de5ae8616f55c3e00239a28d9dd"
UMMS_R5_MARKER = "ONE_UMMS_R5_PREVENTIVE_MAINTENANCE_SCHEDULE_READINESS_PASS"
FREEZE_SCOPE = (
    "PM plan model",
    "PM schedule model",
    "PM task model",
    "PM checklist model",
    "PM trigger type model",
    "Calendar-based PM readiness",
    "Condition-based PM readiness",
    "Work order generation policy model",
    "PM validation gate model",
    "Asset PM linkage model",
    "Airport-to-PM mapping",
    "Customer core function R5 alignment",
...
def _changed_paths() -> set[str]:
    ...
    if _run(["git", "log", "-1", "--pretty=%s"]).stdout.strip() == "docs(one): freeze umms preventive maintenance readiness":
...
def main() -> int:
    ...
    checks.append(("UMMS-R5A freeze document exists", FREEZE_DOC.is_file()))
    checks.append(("Optional registry exists", REGISTRY.is_file()))
    checks.append(("UMMS-R5A report exists", REPORT.is_file()))
    checks.append(("UMMS-R5 commit reference is present", UMMS_R5_COMMIT in freeze_text and UMMS_R5_COMMIT in registry_text))
    checks.append(("UMMS-R5 PASS marker is referenced", UMMS_R5_MARKER in combined))
    checks.append(("Freeze scope includes all required UMMS-R5 scope items", all(item in freeze_text for item in FREEZE_SCOPE)))
    checks.append(("UMMS-R5 summary states UMMS is generic and not Airport-specific", "UMMS remains a generic cross-industry maintenance management module." in freeze_text and "not UMMS core identity" in freeze_text and "not Airport-specific logic" in freeze_text))
    checks.append(("UMMS-R5 summary states no PM execution, due-date runtime calculation, automatic work order generation, workflow execution, or DB write exists", "Automatic work order generation remains disabled." in freeze_text and "No PM execution, PM task execution, due-date runtime calculation, workflow execution, or DB write exists." in freeze_text))
    ...
    checks.append(("No PM schedule creation/update/execution claim", "This is not PM schedule creation, update, or execution." in freeze_text and "pm schedule execution enabled" not in lower_combined))
    checks.append(("No PM task execution claim", "This is not PM task execution." in freeze_text and "pm task execution enabled" not in lower_combined))
    ...
    checks.append(("No state transition execution claim", "This is not state transition execution." in freeze_text and "state transition execution enabled" not in lower_combined))
    ...
    checks.append(("UMMS-R5A PASS marker present", PASS_MARKER in combined))
    ...
    checks.append(("Changes limited to UMMS-R5A freeze scope", not disallowed))
    ...
    if disallowed:
        errors.append(f"changes outside UMMS-R5A scope: {', '.join(sorted(disallowed))}")
    if forbidden:
    ...
    checks.append(("Registry freeze id present", registry_data.get("releaseFreezeId") == "umms-r5a-local-freeze-20260621"))
    ...
    checks.append(("Registry safety matrix frozen", registry_data.get("safetyMatrix", {}).get("readOnly") is True and registry_data.get("safetyMatrix", {}).get("oneAdapterIntroduced") is False and registry_data.get("safetyMatrix", {}).get("pmScheduleExecution") is False))
    checks.append(("Registry validation matrix includes UMMS-R5 marker", UMMS_R5_MARKER in registry_data.get("validationMatrix", {})))
    ...
    checks.append(("Projection JSON validation passes", _json_valid(UMMS_R5_PROJECTION)))
    checks.append(("Registry JSON validation passes", _json_valid(UMMS_R5_REGISTRY) and _json_valid(REGISTRY)))

    print("ONE_UMMS_R5A_LOCAL_FREEZE_VALIDATION")
    for label, ok in checks:
        print(f"[{'PASS' if ok else 'FAIL'}] {label}")
    if errors or not all(ok for _, ok in checks):
        for error in errors:
            print(f"ERROR: {error}", file=sys.stderr)
        print("ONE_UMMS_R5A_LOCAL_FREEZE_AND_OPTIONAL_TAG_PLAN_FAIL")
        return 1
    print(PASS_MARKER)
    return 0
