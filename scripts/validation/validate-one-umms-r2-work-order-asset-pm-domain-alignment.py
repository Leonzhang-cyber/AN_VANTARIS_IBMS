#!/usr/bin/env python3
"""Validate UMMS-R2 work order / asset / PM domain alignment."""
...
PROJECTION = ROOT / "AN_VANTARIS_ONE/projections/umms-work-order-asset-pm-domain-alignment.v1.json"
REGISTRY = ROOT / "AN_VANTARIS_ONE/registries/umms-work-order-asset-pm-domain-alignment-registry.v1.json"
REPORT = ROOT / "ONE_UMMS_R2_WORK_ORDER_ASSET_PM_DOMAIN_ALIGNMENT_REPORT.md"
AIRPORT_CLIENT = ROOT / "AN_VANTARIS_IBMS-frontend/src/services/api/airportGaReadonly.ts"
PASS_MARKER = "ONE_UMMS_R2_WORK_ORDER_ASSET_PM_DOMAIN_ALIGNMENT_PASS"

SAFETY_EXPECTED = {
    "readOnly": True, "runtimeActivation": False, "productionActivation": False,
    "dbWrite": False, "approvalExecution": False, "workflowExecution": False,
    "workOrderCreation": False, "pmScheduleExecution": False, "assetLifecycleWrite": False,
    "inventoryTransaction": False, "vendorContractExecution": False,
    "connectorExecution": False, "deviceConnection": False, "edgeRuntimeCall": False,
    "linkRuntimeCall": False, "oneAdapterIntroduced": False,
}
DOMAIN_OBJECTS = {
    "FaultCase", "WorkOrder", "WorkOrderTrigger", "WorkOrderAssignment", "WorkOrderStatus",
    "WorkOrderEvidence", "Asset", "AssetLifecycleRecord", "AssetLocationReference",
    "MaintenancePlan", "PreventiveMaintenanceSchedule", "PreventiveMaintenanceTask",
    "MaintenanceChecklist", "SparePartRequirement", "InventoryReservation",
    "VendorReference", "ContractReference", "ServiceLevelReference", "HmiLocatorReference",
    "EvidenceReference",
}
WORK_ORDER_STATES = {"draft", "triaged", "assigned", "accepted", "in_progress", "pending_parts", "pending_vendor", "pending_customer", "resolved", "verified", "closed", "cancelled"}
ASSET_STATES = {"planned", "installed", "commissioning", "active", "under_maintenance", "degraded", "retired", "replaced", "decommissioned"}
PM_TRIGGERS = {"calendar_based", "runtime_hours_based", "event_count_based", "fault_count_based", "condition_based", "vendor_recommended", "contract_sla_based"}
AIRPORT_MAPPINGS = {
    "Airport Fault Cases", "Airport Maintenance Work Orders", "Airport Assets & Topology",
    "Airport Alarms & Events", "Airport Evidence Investigation", "Airport Reports",
    "Airport HMI Locator Readiness", "Airport Deployment Readiness",
}
CUSTOMER_FUNCTIONS = {
...
EDGE_DEPENDENCIES = {"Connector matrix", "Source system onboarding profile", "Tag mapping and normalization", "Asset/location mapping preview", "Runtime hours / event counts / condition signals, future", "Engineer diagnostics", "HMI locator data foundation", "Offline deployment readiness"}
LINK_DEPENDENCIES = {"Work-order trigger contract", "Source-system health contract", "Delivery readiness contract", "Asset/location reference contract", "HMI drawing/symbol references", "Audit/evidence chain profile", "Delivery / ACK / retry / DLQ status", "Distributed topology contract", "Remote support bundle contract"}
UCDE_DEPENDENCIES = {"EvidenceRecord", "WorkOrderEvidence", "Closure evidence", "Review evidence", "Audit trail", "Handoff package", "Report evidence"}
ROADMAP = {"UMMS-R3 Manual Work Order Read-only Queue / Draft Model", "UMMS-R4 Work Order Lifecycle State Model + Validation Gate", "UMMS-R5 Preventive Maintenance Schedule Readiness", "UMMS-R6 Spare Parts / Inventory Readiness", "UMMS-R7 Vendor / Contract / SLA Readiness", "UMMS-R8 UMMS + UCDE Evidence Closure Alignment", "UMMS-R9 UMMS + Airport HMI Locator Binding Readiness", "UMMS-R10 UMMS Stakeholder Review Package"}
...
def _changed_paths() -> set[str]:
    ...
    if "umms work asset pm domain alignment" in last_subject:
...
def _run_validator(script: str) -> subprocess.CompletedProcess[str]:
    ...
    spec = importlib.util.spec_from_file_location(f"umms_r2_regression_{path.stem.replace('-', '_')}", path)
...
def main() -> int:
    ...
    checks.append(("UMMS-R2 projection artifact exists", PROJECTION.is_file() and not projection_error))
    checks.append(("UMMS-R2 registry exists", REGISTRY.is_file() and not registry_error))
    checks.append(("UMMS-R2 report exists", REPORT.is_file()))
    if projection_error:
    ...
    summary = projection.get("domainAlignmentSummary", {})
    ...
    checks.append(("domainAlignmentSummary confirms generic UMMS and no Airport-specific core logic", summary.get("genericModuleScope") == "cross_industry_maintenance_management" and summary.get("airportSpecificLogicInCore") is False))
    checks.append(("genericDomainObjects includes all required domain objects", DOMAIN_OBJECTS.issubset({item.get("objectName") for item in projection.get("genericDomainObjects", [])})))
    checks.append(("workOrderDomainModel exists and states no creation/execution", "No work order creation or execution is implemented" in projection.get("workOrderDomainModel", {}).get("statement", "")))
    checks.append(("workOrderLifecycleModel includes all states and no R2 execution", WORK_ORDER_STATES.issubset({item.get("state") for item in projection.get("workOrderLifecycleModel", [])}) and all(item.get("executionAllowedInR2") is False for item in projection.get("workOrderLifecycleModel", []))))
    checks.append(("workOrderTriggerAlignment states LINK provides trigger data and UMMS owns lifecycle", "LINK provides trigger data" in projection.get("workOrderTriggerAlignment", {}).get("statement", "") and "UMMS owns work order lifecycle" in projection.get("workOrderTriggerAlignment", {}).get("statement", "")))
    checks.append(("assetRegistryDomainModel states UMMS-R2 does not write canonical Asset Graph", "does not write canonical Asset Graph" in projection.get("assetRegistryDomainModel", {}).get("statement", "")))
    checks.append(("assetLifecycleModel includes all states and no R2 execution", ASSET_STATES.issubset({item.get("state") for item in projection.get("assetLifecycleModel", [])}) and all(item.get("executionAllowedInR2") is False for item in projection.get("assetLifecycleModel", []))))
    checks.append(("preventiveMaintenanceDomainModel states no PM execution or automatic generation", "No PM schedule execution or automatic work order generation" in projection.get("preventiveMaintenanceDomainModel", {}).get("statement", "")))
    checks.append(("preventiveMaintenanceTriggerModel includes all trigger types and no R2 execution", PM_TRIGGERS.issubset({item.get("triggerType") for item in projection.get("preventiveMaintenanceTriggerModel", [])}) and all(item.get("executionAllowedInR2") is False for item in projection.get("preventiveMaintenanceTriggerModel", []))))
    checks.append(("airportToUmmsMapping includes all required mappings", AIRPORT_MAPPINGS.issubset({item.get("airportSource") for item in projection.get("airportToUmmsMapping", [])})))
    checks.append(("customerCoreFunctionAlignment includes all 10 customer core functions", CUSTOMER_FUNCTIONS.issubset({item.get("function") for item in projection.get("customerCoreFunctionAlignment", [])})))
    ...
    checks.append(("futureImplementationRoadmap includes UMMS-R3 through UMMS-R10", _has_all(projection.get("futureImplementationRoadmap", []), ROADMAP)))
    ...
    checks.append(("Changes limited to UMMS-R2 projection scope", not disallowed and not forbidden))
    if disallowed:
        errors.append(f"changes outside UMMS-R2 scope: {', '.join(sorted(disallowed))}")
    if forbidden:
    ...
    checks.append(("Projection JSON validation passes", not projection_error and metadata.get("projectionId") == "umms-work-order-asset-pm-domain-alignment.v1"))
    ...
    print("ONE_UMMS_R2_WORK_ORDER_ASSET_PM_DOMAIN_ALIGNMENT_VALIDATION")
    for label, ok in checks:
        print(f"[{'PASS' if ok else 'FAIL'}] {label}")
    if errors or not all(ok for _, ok in checks):
        for error in errors:
            print(f"ERROR: {error}", file=sys.stderr)
        print("ONE_UMMS_R2_WORK_ORDER_ASSET_PM_DOMAIN_ALIGNMENT_FAIL")
        return 1
    print(PASS_MARKER)
    return 0
