...
PASS_MARKER = "UHMI_GA_R5_FINAL_CUSTOMER_PREVIEW_READINESS_DECISION_PASS"
REGISTRY = ROOT / "AN_VANTARIS_ONE/registries/uhmi-ga-r5/uhmi-ga-r5-final-customer-preview-readiness-decision.v1.json"
DOCS = [
    ROOT / "UHMI_GA_R5_FINAL_CUSTOMER_PREVIEW_READINESS_MATRIX.md",
    ROOT / "UHMI_GA_R5_RELEASE_DECISION.md",
    ROOT / "UHMI_GA_R5_RISK_LIMITATION_EXCLUSION_MATRIX.md",
    ROOT / "UHMI_GA_R5_CUSTOMER_PREVIEW_SIGNOFF_PACK.md",
    ROOT / "UHMI_GA_R5_REPORT.md",
]
EVIDENCE = [
    ROOT / "AN_VANTARIS_ONE/registries/uhmi-ga-r5/uhmi-r5-readiness-matrix.txt",
    ROOT / "AN_VANTARIS_ONE/registries/uhmi-ga-r5/uhmi-r5-release-decision.txt",
    ROOT / "AN_VANTARIS_ONE/registries/uhmi-ga-r5/uhmi-r5-release-chain.txt",
    ROOT / "AN_VANTARIS_ONE/registries/uhmi-ga-r5/uhmi-r5-risk-exclusion-evidence.txt",
    ROOT / "AN_VANTARIS_ONE/registries/uhmi-ga-r5/uhmi-r5-validator-results.txt",
]
...
EXPECTED_CHAIN = {
    ...
    "UHMI-GA-R4": {
        "commit": "138cb73906ef1026e34461bf1eb0583bb3878d9e",
        "tag": "uhmi-ga-r4-customer-preview-export-handoff-freeze-20260622",
        "marker": "UHMI_GA_R4_CUSTOMER_PREVIEW_EXPORT_PACKAGE_PASS",
        "registry": "AN_VANTARIS_ONE/registries/uhmi-ga-r4/uhmi-ga-r4-customer-preview-export-package.v1.json",
    },
}
...
def main() -> None:
    for path in [*DOCS, *EVIDENCE, REGISTRY]:
        if not path.exists():
            fail(f"missing required R5 file: {path.relative_to(ROOT)}")
    ok("required R5 documents, evidence files, and registry exist")
    ...
    if registry.get("finalCustomerPreviewReadinessMatrix") is not True:
        fail("registry.finalCustomerPreviewReadinessMatrix must be true")
    ok("R5 registry core fields are correct")
    ...
    ok("releaseChain includes R1-R4 with correct commits, tags, scopes, status, and PASS markers")
    ...
    for doc in DOCS:
        if PASS_MARKER not in read(doc):
            fail(f"R5 PASS marker missing from {doc.relative_to(ROOT)}")
    required_doc_phrases = [
        "Final Customer Preview Readiness Matrix",
        "Release Decision",
        "GO",
    ...
    for phrase in required_doc_phrases:
        if phrase not in combined_docs:
            fail(f"required R5 doc phrase missing: {phrase}")
    ok("R5 docs contain required readiness, decision, style, and boundary phrases")
    ...
    for path in [*DOCS, *EVIDENCE, REGISTRY, ROOT / "scripts/validation/validate-uhmi-ga-r5-final-customer-preview-readiness-decision.py"]:
        relative = path.relative_to(ROOT).as_posix()
        for token in [".env", "node_modules", "dist", "build", ".runtime", "secrets", "tar.gz", "zip"]:
            if token in relative:
                fail(f"forbidden artifact path in R5 files: {relative}")
    ok("no forbidden artifact roots, archives, or files were added by R5")
