<template>
  ...
  <div v-if="!isLoaded" class="loading-container">
    <div class="loading-overlay">
      <div class="loading-content">
        ...
        <div class="loading-tip">Vertiv Device Drivers</div>
  ...
  <div v-else class="vertiv-page">
      ...
      <div>
        <div class="breadcrumb">
          <el-breadcrumb separator="/">
            ...
            <el-breadcrumb-item>Vertiv</el-breadcrumb-item>
          </el-breadcrumb>
        </div>
        <h1>Vertiv Device Drivers</h1>
        <p class="description">Manage Vertiv UPS, cooling systems, PDU, and data center infrastructure monitoring</p>
      </div>
    ...
    <!-- Vertiv Devices Table -->
    <el-card class="devices-card" shadow="hover">
      <template #header>
        <div class="card-header">
          <span>Vertiv Devices</span>
          <div class="table-actions">
            ...
            <el-select v-model="typeFilter" placeholder="Device Type" clearable style="width: 140px">
              ...
              <el-option label="Rectifier" value="Rectifier" />
            </el-select>
      ...
      <el-table :data="paginatedDevices" stripe style="width: 100%" v-loading="tableLoading">
        ...
        <el-table-column prop="parameterCount" label="Parameters" width="90" align="center" />
    ...
    <el-card class="parameters-card" shadow="hover" v-if="selectedDevice">
      ...
      <el-table :data="parameters" stripe size="small" v-loading="paramLoading">
        <el-table-column prop="parameterId" label="Parameter ID" width="120" />
        ...
        <el-table-column prop="alarmStatus" label="Alarm Status" width="110">
          <template #default="{ row }">
            <el-tag :type="row.alarmStatus === 'Normal' ? 'success' : row.alarmStatus === 'Warning' ? 'warning' : 'danger'" size="small">
              {{ row.alarmStatus }}
            </el-tag>
          </template>
        </el-table-column>
        ...
        <el-table-column label="Actions" width="120">
          <template #default="{ row }">
            ...
            <el-button v-if="isWritable(row)" link type="success" size="small" @click="openWriteDialog(row)">Write</el-button>
          </template>
        </el-table-column>
      </el-table>
      ...
      <div class="param-footer" v-if="selectedDevice">
        ...
        <span class="param-info">Total Parameters: {{ parameters.length }} | Alarms: {{ activeAlarmsCount }}</span>
      </div>
    </el-card>

    <!-- Add/Edit Device Dialog -->
    <el-dialog v-model="deviceDialogVisible" :title="dialogMode === 'add' ? 'Add Vertiv Device' : 'Edit Vertiv Device'" width="650px" destroy-on-close class="device-dialog">
        ...
        <el-tab-pane label="Connection Settings" name="basic">
          <el-form :model="deviceForm" :rules="deviceRules" ref="deviceFormRef" label-width="140px">
            <el-row :gutter="20">
              ...
              <el-col :span="12">
                <el-form-item label="Model" prop="model">
                  <el-select v-model="deviceForm.model" style="width: 100%">
                    <el-option label="Liebert EXM" value="Liebert EXM" />
                    <el-option label="Liebert EXS" value="Liebert EXS" />
                    <el-option label="Liebert APM" value="Liebert APM" />
                    <el-option label="Liebert NXC" value="Liebert NXC" />
                    <el-option label="Liebert CRV" value="Liebert CRV" />
                    <el-option label="Liebert DSE" value="Liebert DSE" />
                    <el-option label="Liebert PPC" value="Liebert PPC" />
                    <el-option label="Liebert MPX" value="Liebert MPX" />
                    <el-option label="Liebert RDU" value="Liebert RDU" />
                  </el-select>
                </el-form-item>
              </el-col>
              <el-col :span="12">
              ...
              <el-col :span="12">
                <el-form-item label="Community/Unit ID" prop="community">
                  <el-input v-model="deviceForm.community" placeholder="public or unit ID" />
                </el-form-item>
              </el-col>
            </el-row>
    ...
    <el-dialog v-model="parameterDialogVisible" :title="dialogMode === 'add' ? 'Add Parameter' : 'Edit Parameter'" width="550px" destroy-on-close>
      <el-form :model="parameterForm" :rules="parameterRules" ref="parameterFormRef" label-width="120px">
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="Parameter ID" prop="parameterId">
              <el-input v-model="parameterForm.parameterId" placeholder="e.g., UPS.1.1" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
          ...
          <el-col :span="12">
            <el-form-item label="Unit" prop="unit">
              <el-input v-model="parameterForm.unit" placeholder="kW, %, min, V, A" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="Warning High" prop="warningHigh">
              <el-input-number v-model="parameterForm.warningHigh" style="width: 100%" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="Critical High" prop="criticalHigh">
              <el-input-number v-model="parameterForm.criticalHigh" style="width: 100%" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="Warning Low" prop="warningLow">
              <el-input-number v-model="parameterForm.warningLow" style="width: 100%" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="Critical Low" prop="criticalLow">
              <el-input-number v-model="parameterForm.criticalLow" style="width: 100%" />
            </el-form-item>
          </el-col>
          <el-col :span="24">
            <el-form-item label="Description" prop="description">
...
import {
  ...
  Delete, Connection, Edit, Cpu, DataAnalysis, Warning
...
const loadingMessages = ['Preparing...', 'Initializing Vertiv drivers...', 'Loading devices...', 'Almost ready...']
...
const statsCards = ref([
  { title: 'Vertiv Devices', value: '12', trend: 3, icon: 'Cpu', bgColor: '#409eff', key: 'devices', subTitle: 'Online: 11' },
  { title: 'Parameters', value: '186', trend: 8, icon: 'DataAnalysis', bgColor: '#67c23a', key: 'params', subTitle: 'Active: 178' },
  { title: 'Active Alarms', value: '3', trend: -2, icon: 'Warning', bgColor: '#e6a23c', key: 'alarms', subTitle: 'Critical: 1' },
  { title: 'Data Points', value: '2.4K/s', trend: 12, icon: 'TrendCharts', bgColor: '#f56c6c', key: 'rate', subTitle: 'Peak: 3.2K/s' }
])

const devices = ref([
  { id: 1, name: 'Main_UPS_1', deviceType: 'UPS', model: 'Liebert EXM', ipAddress: '192.168.1.100', port: 161, protocol: 'SNMP', community: 'public', parameterCount: 45, status: 'Online', lastSeen: '2024-01-20 10:30:00' },
  { id: 2, name: 'Data_Center_Cooling', deviceType: 'Cooling', model: 'Liebert CRV', ipAddress: '192.168.1.101', port: 502, protocol: 'Modbus TCP', community: '1', parameterCount: 32, status: 'Online', lastSeen: '2024-01-20 10:28:00' },
  { id: 3, name: 'Server_Room_PDU_A', deviceType: 'PDU', model: 'Liebert MPX', ipAddress: '192.168.1.102', port: 161, protocol: 'SNMP', community: 'public', parameterCount: 24, status: 'Online', lastSeen: '2024-01-20 10:32:00' },
  { id: 4, name: 'Environmental_Monitor', deviceType: 'Monitoring', model: 'Liebert RDU', ipAddress: '192.168.1.103', port: 502, protocol: 'Modbus TCP', community: '1', parameterCount: 18, status: 'Warning', lastSeen: '2024-01-20 10:25:00' },
  { id: 5, name: 'Rectifier_System', deviceType: 'Rectifier', model: 'Liebert NXC', ipAddress: '192.168.1.104', port: 161, protocol: 'SNMP', community: 'public', parameterCount: 28, status: 'Online', lastSeen: '2024-01-20 10:29:00' }
])

const parametersMap = ref<Record<number, any[]>>({
  1: [
    { id: 1, parameterId: 'UPS.1.1', name: 'Input Voltage', dataType: 'Numeric', unit: 'V', currentValue: 480, alarmStatus: 'Normal', warningHigh: 500, criticalHigh: 520, warningLow: 440, criticalLow: 400, description: 'UPS input voltage' },
    { id: 2, parameterId: 'UPS.1.2', name: 'Output Voltage', dataType: 'Numeric', unit: 'V', currentValue: 480, alarmStatus: 'Normal', warningHigh: 500, criticalHigh: 520, warningLow: 440, criticalLow: 400, description: 'UPS output voltage' },
    { id: 3, parameterId: 'UPS.1.3', name: 'Load Percentage', dataType: 'Numeric', unit: '%', currentValue: 45, alarmStatus: 'Normal', warningHigh: 80, criticalHigh: 90, warningLow: 10, criticalLow: 5, description: 'UPS load percentage' },
    { id: 4, parameterId: 'UPS.1.4', name: 'Battery Capacity', dataType: 'Numeric', unit: '%', currentValue: 95, alarmStatus: 'Normal', warningHigh: 0, criticalHigh: 0, warningLow: 30, criticalLow: 20, description: 'Battery remaining capacity' },
    { id: 5, parameterId: 'UPS.1.5', name: 'Temperature', dataType: 'Numeric', unit: '°C', currentValue: 28.5, alarmStatus: 'Normal', warningHigh: 35, criticalHigh: 40, warningLow: 0, criticalLow: 0, description: 'UPS internal temperature' }
  ],
  2: [
    { id: 6, parameterId: 'CRV.1.1', name: 'Supply Air Temp', dataType: 'Numeric', unit: '°C', currentValue: 18.5, alarmStatus: 'Normal', warningHigh: 25, criticalHigh: 30, warningLow: 15, criticalLow: 10, description: 'Cooling supply air temperature' },
    { id: 7, parameterId: 'CRV.1.2', name: 'Fan Speed', dataType: 'Numeric', unit: '%', currentValue: 65, alarmStatus: 'Normal', warningHigh: 90, criticalHigh: 100, warningLow: 0, criticalLow: 0, description: 'Fan speed percentage' }
  ]
})
...
const activeAlarmsCount = computed(() => parameters.value.filter(p => p.alarmStatus !== 'Normal').length)
...
const isWritable = (param: any) => {
  // Some parameters like setpoints can be written
  return param.parameterId.includes('Setpoint') || param.parameterId.includes('Command')
}

const handleCardClick = (stat: any) => ElMessage.info(`Viewing ${stat.title} details`)
const handleExport = () => ElMessage.success('Exporting Vertiv configuration...')
...
const openAddDeviceDialog = () => {
  ...
  Object.assign(deviceForm, {
    id: null, name: '', deviceType: 'UPS', model: 'Liebert EXM', ipAddress: '', port: 161,
    protocol: 'SNMP', community: 'public', description: '', timeout: 5000, retryCount: 3,
...
const testDeviceConnection = () => {
  ...
  setTimeout(() => {
    ...
    testResult.message = 'Successfully connected to Vertiv device'
    testResult.details = {
...
const openAddParameterDialog = () => {
  ...
  Object.assign(parameterForm, {
    id: null, parameterId: '', name: '', dataType: 'Numeric', unit: '',
    warningHigh: 0, criticalHigh: 0, warningLow: 0, criticalLow: 0, description: ''
  })
...
const readParameter = (param: any) => {
  ...
  setTimeout(() => {
    const newValue = param.dataType === 'Numeric' ? (Math.random() * 100).toFixed(2) : Math.random() > 0.5
    param.currentValue = newValue
    // Update alarm status based on thresholds
    if (param.dataType === 'Numeric') {
      const val = parseFloat(newValue)
      if (param.criticalHigh && val >= param.criticalHigh) param.alarmStatus = 'Critical'
      else if (param.criticalLow && val <= param.criticalLow) param.alarmStatus = 'Critical'
      else if (param.warningHigh && val >= param.warningHigh) param.alarmStatus = 'Warning'
      else if (param.warningLow && val <= param.warningLow) param.alarmStatus = 'Warning'
      else param.alarmStatus = 'Normal'
    }
  ...
  setTimeout(() => {
    parameters.value.forEach(param => {
      if (param.dataType === 'Numeric') {
        ...
        param.currentValue = Math.random() > 0.5
      }
    })
...
const parameterRules = {
  parameterId: [{ required: true, message: 'Please enter parameter ID', trigger: 'blur' }],
...
.write-param, .write-current { font-family: monospace; font-size: 13px; }
