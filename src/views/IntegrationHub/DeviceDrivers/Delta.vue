  ...
  <div v-if="!isLoaded" class="loading-container">
    <div class="loading-overlay">
      <div class="loading-content">
        ...
        <div class="loading-tip">Delta Device Drivers</div>
  ...
  <div v-else class="delta-page">
      ...
      <div>
        <div class="breadcrumb">
          <el-breadcrumb separator="/">
            ...
            <el-breadcrumb-item>Delta</el-breadcrumb-item>
          </el-breadcrumb>
        </div>
        <h1>Delta Device Drivers</h1>
        <p class="description">Manage Delta PLCs, drives, power systems, and industrial automation equipment</p>
      </div>
    ...
    <el-card class="devices-card" shadow="hover">
        ...
        <div class="card-header">
          ...
          <div class="table-actions">
            ...
            <el-select v-model="typeFilter" placeholder="Device Type" clearable style="width: 140px">
              ...
              <el-option label="VFD" value="VFD" />
              <el-option label="Servo" value="Servo" />
      ...
      <el-table :data="paginatedDevices" stripe style="width: 100%" v-loading="tableLoading">
        ...
        <el-table-column prop="deviceType" label="Type" width="110">
    ...
    <el-card class="parameters-card" shadow="hover" v-if="selectedDevice">
      ...
      <el-table :data="parameters" stripe size="small" v-loading="paramLoading">
        ...
        <el-table-column prop="range" label="Range" width="120" />
      ...
      <div class="param-footer" v-if="selectedDevice">
        ...
        <span class="param-info">Total Parameters: {{ parameters.length }} | Mapped: {{ mappedCount }}</span>
      </div>
    </el-card>

    <!-- Add/Edit Device Dialog -->
    <el-dialog v-model="deviceDialogVisible" :title="dialogMode === 'add' ? 'Add Delta Device' : 'Edit Delta Device'" width="650px" destroy-on-close class="device-dialog">
              ...
              <el-col :span="12">
                <el-form-item label="Model" prop="model">
                  <el-select v-model="deviceForm.model" style="width: 100%">
                    <el-option label="DVP-ES2" value="DVP-ES2" />
                    <el-option label="DVP-SV2" value="DVP-SV2" />
                    <el-option label="DVP-EH3" value="DVP-EH3" />
                    <el-option label="DVP-EC3" value="DVP-EC3" />
                    <el-option label="VFD-EL" value="VFD-EL" />
                    <el-option label="VFD-C2000" value="VFD-C2000" />
                    <el-option label="VFD-E" value="VFD-E" />
                    <el-option label="ASDA-A2" value="ASDA-A2" />
                    <el-option label="ASDA-B2" value="ASDA-B2" />
                    <el-option label="Ultron UPS" value="Ultron UPS" />
                    <el-option label="DPR UPS" value="DPR UPS" />
                    <el-option label="PMT Power" value="PMT Power" />
                    <el-option label="DOP HMI" value="DOP HMI" />
                  </el-select>
                </el-form-item>
              </el-col>
              <el-col :span="12">
              ...
              <el-col :span="12">
                <el-form-item label="Protocol" prop="protocol">
                  <el-select v-model="deviceForm.protocol" style="width: 100%">
                    ...
                    <el-option label="Delta ASCII" value="Delta ASCII" />
                    <el-option label="EtherNet/IP" value="EtherNet/IP" />
                  </el-select>
                </el-form-item>
              </el-col>
              <el-col :span="12">
                <el-form-item label="Station ID" prop="stationId">
                  <el-input-number v-model="deviceForm.stationId" :min="0" :max="255" style="width: 100%" />
                </el-form-item>
              </el-col>
            </el-row>
        ...
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="Address" prop="address">
              <el-input v-model="parameterForm.address" placeholder="e.g., D0, 40001" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
          ...
          <el-col :span="12">
            <el-form-item label="Unit" prop="unit">
              <el-input v-model="parameterForm.unit" placeholder="Hz, %, rpm, V" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
          ...
          <el-col :span="12">
            <el-form-item label="Read/Write" prop="access">
              <el-select v-model="parameterForm.access" style="width: 100%">
                <el-option label="Read Only" value="RO" />
                <el-option label="Read/Write" value="RW" />
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="24">
...
const loadingMessages = ['Preparing...', 'Initializing Delta drivers...', 'Loading devices...', 'Almost ready...']
...
const statsCards = ref([
  { title: 'Delta Devices', value: '15', trend: 4, icon: 'Cpu', bgColor: '#409eff', key: 'devices', subTitle: 'Online: 14' },
  { title: 'Parameters', value: '234', trend: 12, icon: 'DataAnalysis', bgColor: '#67c23a', key: 'params', subTitle: 'Active: 228' },
  { title: 'Data Rate', value: '856/s', trend: 6, icon: 'TrendCharts', bgColor: '#e6a23c', key: 'rate', subTitle: 'Peak: 1.2K/s' },
  { title: 'Alarms', value: '2', trend: -1, icon: 'Warning', bgColor: '#f56c6c', key: 'alarms', subTitle: 'Active: 1' }
])

const devices = ref([
  { id: 1, name: 'Main_Production_PLC', deviceType: 'PLC', model: 'DVP-ES2', ipAddress: '192.168.1.100', port: 502, protocol: 'Modbus TCP', stationId: 1, parameterCount: 45, status: 'Online', lastSeen: '2024-01-20 10:30:00' },
  { id: 2, name: 'Conveyor_VFD', deviceType: 'VFD', model: 'VFD-C2000', ipAddress: '192.168.1.101', port: 502, protocol: 'Modbus TCP', stationId: 2, parameterCount: 32, status: 'Online', lastSeen: '2024-01-20 10:28:00' },
  { id: 3, name: 'Servo_Drive_1', deviceType: 'Servo', model: 'ASDA-A2', ipAddress: '192.168.1.102', port: 502, protocol: 'Modbus TCP', stationId: 3, parameterCount: 28, status: 'Online', lastSeen: '2024-01-20 10:32:00' },
  { id: 4, name: 'Data_Center_UPS', deviceType: 'UPS', model: 'Ultron UPS', ipAddress: '192.168.1.103', port: 502, protocol: 'Modbus TCP', stationId: 4, parameterCount: 24, status: 'Warning', lastSeen: '2024-01-20 10:25:00' },
  { id: 5, name: 'Power_Supply_01', deviceType: 'Power Supply', model: 'PMT Power', ipAddress: '192.168.1.104', port: 502, protocol: 'Modbus TCP', stationId: 5, parameterCount: 18, status: 'Online', lastSeen: '2024-01-20 10:29:00' },
  { id: 6, name: 'HMI_Control', deviceType: 'HMI', model: 'DOP HMI', ipAddress: '192.168.1.105', port: 502, protocol: 'Modbus TCP', stationId: 6, parameterCount: 22, status: 'Online', lastSeen: '2024-01-20 10:31:00' }
])

const parametersMap = ref<Record<number, any[]>>({
  1: [
    { id: 1, address: 'D0', name: 'Production Count', dataType: 'DWord', unit: 'pcs', currentValue: 12450, range: '0-999999', access: 'RW', scaleFactor: 1, description: 'Total production count' },
    { id: 2, address: 'D100', name: 'Motor Speed', dataType: 'Word', unit: 'rpm', currentValue: 1450, range: '0-3000', access: 'RW', scaleFactor: 1, description: 'Motor speed setpoint' },
    { id: 3, address: 'D200', name: 'Temperature', dataType: 'Float', unit: '°C', currentValue: 22.5, range: '-50-150', access: 'RO', scaleFactor: 0.1, description: 'Process temperature' },
    { id: 4, address: 'M0', name: 'System Ready', dataType: 'Boolean', unit: '', currentValue: true, range: '0/1', access: 'RO', scaleFactor: 1, description: 'System ready status' }
  ],
  2: [
    { id: 5, address: '2101', name: 'Output Frequency', dataType: 'Word', unit: 'Hz', currentValue: 50.0, range: '0-120', access: 'RW', scaleFactor: 0.1, description: 'VFD output frequency' },
    { id: 6, address: '2102', name: 'Output Current', dataType: 'Word', unit: 'A', currentValue: 12.5, range: '0-100', access: 'RO', scaleFactor: 0.1, description: 'Motor current' }
  ],
  3: [
    { id: 7, address: 'P0-02', name: 'Position Command', dataType: 'DWord', unit: 'pulse', currentValue: 10000, range: '0-999999', access: 'RW', scaleFactor: 1, description: 'Servo position command' },
    { id: 8, address: 'P0-03', name: 'Actual Speed', dataType: 'Word', unit: 'rpm', currentValue: 1200, range: '0-5000', access: 'RO', scaleFactor: 1, description: 'Actual motor speed' }
  ]
})
...
const mappedCount = computed(() => parameters.value.length)
...
const isWritable = (param: any) => {
  return param.access === 'RW'
}

const handleCardClick = (stat: any) => ElMessage.info(`Viewing ${stat.title} details`)
const handleExport = () => ElMessage.success('Exporting Delta configuration...')
...
const openAddDeviceDialog = () => {
  ...
  Object.assign(deviceForm, {
    id: null, name: '', deviceType: 'PLC', model: 'DVP-ES2', ipAddress: '', port: 502,
    protocol: 'Modbus TCP', stationId: 1, description: '', timeout: 5000, retryCount: 3,
...
const testDeviceConnection = () => {
  ...
  setTimeout(() => {
    ...
    testResult.message = 'Successfully connected to Delta device'
    testResult.details = {
...
const openAddParameterDialog = () => {
  ...
  Object.assign(parameterForm, {
    id: null, address: '', name: '', dataType: 'Word', unit: '',
    scaleFactor: 1, access: 'RW', description: ''
  })
...
const readParameter = (param: any) => {
  ...
  setTimeout(() => {
    const newValue = param.dataType === 'Float' ? (Math.random() * 100).toFixed(2) :
        param.dataType === 'Boolean' ? Math.random() > 0.5 :
...
const readAllParameters = () => {
  ...
  setTimeout(() => {
    parameters.value.forEach(param => {
      param.currentValue = param.dataType === 'Float' ? (Math.random() * 100).toFixed(2) :
          param.dataType === 'Boolean' ? Math.random() > 0.5 :
              Math.floor(Math.random() * 1000)
    })
...
.write-param, .write-address, .write-current { font-family: monospace; font-size: 13px; }
