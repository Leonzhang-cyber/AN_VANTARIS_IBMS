<template>
  ...
  <div v-if="!isLoaded" class="loading-container">
    <div class="loading-overlay">
      <div class="loading-content">
        ...
        <div class="loading-tip">Mitsubishi Device Drivers</div>
  ...
  <!-- Main Content -->
  <div v-else class="mitsubishi-page">
    <!-- Page Header -->
      ...
      <div>
        <div class="breadcrumb">
          <el-breadcrumb separator="/">
            ...
            <el-breadcrumb-item>Mitsubishi</el-breadcrumb-item>
          </el-breadcrumb>
        </div>
        <h1>Mitsubishi Device Drivers</h1>
        <p class="description">Manage Mitsubishi PLCs, VFDs, servos, CNCs, and industrial automation equipment</p>
      </div>
    ...
    <!-- Mitsubishi Devices Table -->
    <el-card class="devices-card" shadow="hover">
      <template #header>
        <div class="card-header">
          <span>Mitsubishi Devices</span>
    ...
    <el-card class="points-card" shadow="hover" v-if="selectedDevice">
      <template #header>
        <div class="card-header">
          <span>Device Points - {{ selectedDevice.name }}</span>
      ...
      <el-table :data="points" stripe size="small" v-loading="pointsLoading">
        ...
        <el-table-column label="Actions" width="150">
          <template #default="{ row }">
            ...
            <el-button v-if="isWritable(row.address)" link type="success" size="small" @click="openWriteDialog(row)">Write</el-button>
    ...
    <el-dialog v-model="deviceDialogVisible" :title="dialogMode === 'add' ? 'Add Mitsubishi Device' : 'Edit Mitsubishi Device'" width="650px" destroy-on-close class="device-dialog">
        ...
        <el-tab-pane label="Connection Settings" name="basic">
          <el-form :model="deviceForm" :rules="deviceRules" ref="deviceFormRef" label-width="140px">
            <el-row :gutter="20">
              ...
              <el-col :span="12">
                <el-form-item label="Model" prop="model">
                  <el-select v-model="deviceForm.model" style="width: 100%">
                    <el-option label="MELSEC iQ-R" value="MELSEC iQ-R" />
                    <el-option label="MELSEC iQ-F" value="MELSEC iQ-F" />
                    <el-option label="MELSEC Q" value="MELSEC Q" />
                    <el-option label="MELSEC L" value="MELSEC L" />
                    <el-option label="FX5U" value="FX5U" />
                    <el-option label="FX3U" value="FX3U" />
                    <el-option label="FR-A800" value="FR-A800" />
                    <el-option label="FR-E800" value="FR-E800" />
                    <el-option label="MELSERVO J5" value="MELSERVO J5" />
                    <el-option label="MELSERVO J4" value="MELSERVO J4" />
                    <el-option label="MELFA" value="MELFA" />
                    <el-option label="GOT2000" value="GOT2000" />
                  </el-select>
                </el-form-item>
              </el-col>
              <el-col :span="12">
              ...
              <el-col :span="12">
                <el-form-item label="Protocol" prop="protocol">
                  <el-select v-model="deviceForm.protocol" style="width: 100%">
                    <el-option label="MELSEC" value="MELSEC" />
                    <el-option label="SLMP" value="SLMP" />
              ...
              <el-col :span="12">
                <el-form-item label="Station No." prop="stationNo">
                  <el-input-number v-model="deviceForm.stationNo" :min="0" :max="255" style="width: 100%" />
                </el-form-item>
              </el-col>
            </el-row>
    ...
    <el-dialog v-model="pointDialogVisible" :title="dialogMode === 'add' ? 'Add Point' : 'Edit Point'" width="550px" destroy-on-close>
      <el-form :model="pointForm" :rules="pointRules" ref="pointFormRef" label-width="120px">
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="Address" prop="address">
              <el-input v-model="pointForm.address" placeholder="D100, X0, Y10" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
          ...
          <el-col :span="12">
            <el-form-item label="Unit" prop="unit">
              <el-input v-model="pointForm.unit" placeholder="rpm, %, Hz" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
...
const loadingMessages = ['Preparing...', 'Initializing Mitsubishi drivers...', 'Loading devices...', 'Almost ready...']
...
const statsCards = ref([
  { title: 'Mitsubishi Devices', value: '18', trend: 6, icon: 'Cpu', bgColor: '#409eff', key: 'devices', subTitle: 'Online: 16' },
  { title: 'Data Points', value: '456', trend: 14, icon: 'DataAnalysis', bgColor: '#67c23a', key: 'points', subTitle: 'Active: 438' },
  { title: 'Scan Rate', value: '1.2K/s', trend: 8, icon: 'TrendCharts', bgColor: '#e6a23c', key: 'rate', subTitle: 'Peak: 1.8K/s' },
  { title: 'Active Alarms', value: '2', trend: -1, icon: 'Warning', bgColor: '#f56c6c', key: 'alarms', subTitle: 'Unresolved' }
])

const devices = ref([
  { id: 1, name: 'Main_PLC_1', deviceType: 'PLC', model: 'MELSEC iQ-R', ipAddress: '192.168.1.100', port: 5001, protocol: 'SLMP', stationNo: 1, pointCount: 86, status: 'Online', lastSeen: '2024-01-20 10:30:00' },
  { id: 2, name: 'Conveyor_VFD', deviceType: 'VFD', model: 'FR-A800', ipAddress: '192.168.1.101', port: 502, protocol: 'Modbus TCP', stationNo: 2, pointCount: 32, status: 'Online', lastSeen: '2024-01-20 10:28:00' },
  { id: 3, name: 'Servo_Drive_1', deviceType: 'Servo', model: 'MELSERVO J5', ipAddress: '192.168.1.102', port: 5001, protocol: 'SLMP', stationNo: 3, pointCount: 28, status: 'Online', lastSeen: '2024-01-20 10:32:00' },
  { id: 4, name: 'CNC_Machine', deviceType: 'CNC', model: 'MELFA', ipAddress: '192.168.1.103', port: 5001, protocol: 'MELSEC', stationNo: 4, pointCount: 45, status: 'Warning', lastSeen: '2024-01-20 10:25:00' },
  { id: 5, name: 'Robot_Arm', deviceType: 'Robot', model: 'MELFA', ipAddress: '192.168.1.104', port: 5001, protocol: 'SLMP', stationNo: 5, pointCount: 38, status: 'Online', lastSeen: '2024-01-20 10:29:00' },
  { id: 6, name: 'HMI_Control', deviceType: 'HMI', model: 'GOT2000', ipAddress: '192.168.1.105', port: 5001, protocol: 'MELSEC', stationNo: 6, pointCount: 24, status: 'Online', lastSeen: '2024-01-20 10:31:00' }
])

const pointsMap = ref<Record<number, any[]>>({
  1: [
    { id: 1, address: 'D100', name: 'Production Count', dataType: 'DWord', unit: 'pcs', currentValue: 12450, range: '0-999999', scaleFactor: 1, minValue: 0, maxValue: 999999, description: 'Total production count' },
    { id: 2, address: 'D200', name: 'Line Speed', dataType: 'Word', unit: 'rpm', currentValue: 1450, range: '0-3000', scaleFactor: 1, minValue: 0, maxValue: 3000, description: 'Conveyor line speed' },
    { id: 3, address: 'D300', name: 'Temperature', dataType: 'Float', unit: '°C', currentValue: 22.5, range: '-50-150', scaleFactor: 0.1, minValue: -50, maxValue: 150, description: 'Process temperature' },
    { id: 4, address: 'X0', name: 'System Ready', dataType: 'Boolean', unit: '', currentValue: true, range: '0/1', scaleFactor: 1, description: 'System ready status' },
    { id: 5, address: 'Y10', name: 'Motor Start', dataType: 'Boolean', unit: '', currentValue: true, range: '0/1', scaleFactor: 1, description: 'Motor start command' }
  ],
  2: [
    { id: 6, address: '40001', name: 'Output Frequency', dataType: 'Float', unit: 'Hz', currentValue: 50.0, range: '0-120', scaleFactor: 0.1, minValue: 0, maxValue: 120, description: 'VFD output frequency' },
    { id: 7, address: '40003', name: 'Output Current', dataType: 'Float', unit: 'A', currentValue: 12.5, range: '0-100', scaleFactor: 0.1, minValue: 0, maxValue: 100, description: 'Motor current' }
  ],
  3: [
    { id: 8, address: 'D0', name: 'Position Command', dataType: 'DWord', unit: 'pulse', currentValue: 10000, range: '0-999999', scaleFactor: 1, minValue: 0, maxValue: 999999, description: 'Servo position command' },
    { id: 9, address: 'D2', name: 'Actual Speed', dataType: 'Word', unit: 'rpm', currentValue: 1200, range: '0-5000', scaleFactor: 1, minValue: 0, maxValue: 5000, description: 'Actual motor speed' }
  ]
})
...
const isWritable = (address: string) => {
  // D registers and Y outputs are writable, X inputs are read-only
  return address.startsWith('D') || address.startsWith('Y') || address.startsWith('M')
}

const handleCardClick = (stat: any) => ElMessage.info(`Viewing ${stat.title} details`)
const handleExport = () => ElMessage.success('Exporting Mitsubishi configuration...')
...
const openAddDeviceDialog = () => {
  ...
  Object.assign(deviceForm, {
    id: null, name: '', deviceType: 'PLC', model: 'MELSEC iQ-R', ipAddress: '', port: 5001,
    protocol: 'SLMP', stationNo: 1, description: '', timeout: 5000, retryCount: 3,
...
const testDeviceConnection = () => {
  ...
  setTimeout(() => {
    ...
    testResult.message = 'Successfully connected to Mitsubishi device'
    testResult.details = {
...
const readPoint = (point: any) => {
  ...
  setTimeout(() => {
    const newValue = point.dataType === 'Float' ? (Math.random() * 100).toFixed(2) :
...
const readAllPoints = () => {
  ...
  setTimeout(() => {
    points.value.forEach(point => {
      point.currentValue = point.dataType === 'Float' ? (Math.random() * 100).toFixed(2) :
          point.dataType === 'Boolean' ? Math.random() > 0.5 :
              Math.floor(Math.random() * 1000)
    })
