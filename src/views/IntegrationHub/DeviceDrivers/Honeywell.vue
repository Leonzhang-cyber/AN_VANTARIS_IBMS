<template>
  ...
  <div v-if="!isLoaded" class="loading-container">
    <div class="loading-overlay">
      <div class="loading-content">
        ...
        <div class="loading-tip">Honeywell Device Drivers</div>
  ...
  <!-- Main Content -->
  <div v-else class="honeywell-page">
    <!-- Page Header -->
      ...
      <div>
        <div class="breadcrumb">
          <el-breadcrumb separator="/">
            ...
            <el-breadcrumb-item>Honeywell</el-breadcrumb-item>
          </el-breadcrumb>
        </div>
        <h1>Honeywell Device Drivers</h1>
        <p class="description">Manage Honeywell PLCs, controllers, and building automation devices</p>
      </div>
    ...
    <!-- Honeywell Devices Table -->
    <el-card class="devices-card" shadow="hover">
      <template #header>
        <div class="card-header">
          <span>Honeywell Devices</span>
    ...
    <el-dialog v-model="deviceDialogVisible" :title="dialogMode === 'add' ? 'Add Honeywell Device' : 'Edit Honeywell Device'" width="650px" destroy-on-close class="device-dialog">
              ...
              <el-col :span="12">
                <el-form-item label="Model" prop="model">
                  <el-select v-model="deviceForm.model" style="width: 100%">
                    <el-option label="C300" value="C300" />
                    <el-option label="C200" value="C200" />
                    <el-option label="HC900" value="HC900" />
                    <el-option label="MasterLogic" value="MasterLogic" />
                    <el-option label="Lyric T6" value="Lyric T6" />
                    <el-option label="Lyric T5" value="Lyric T5" />
                    <el-option label="Rondostat" value="Rondostat" />
                    <el-option label="WEB-9000" value="WEB-9000" />
                    <el-option label="WEB-8000" value="WEB-8000" />
                  </el-select>
                </el-form-item>
              </el-col>
              <el-col :span="12">
    ...
    <el-dialog v-model="pointDialogVisible" :title="dialogMode === 'add' ? 'Add Data Point' : 'Edit Data Point'" width="600px" destroy-on-close>
      <el-form :model="pointForm" :rules="pointRules" ref="pointFormRef" label-width="120px">
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="Address" prop="address">
              <el-input v-model="pointForm.address" placeholder="e.g., 40001, AI-1" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
          ...
          <el-col :span="12">
            <el-form-item label="Unit" prop="unit">
              <el-input v-model="pointForm.unit" placeholder="°C, %, kPa" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
...
const loadingMessages = ['Preparing...', 'Initializing Honeywell drivers...', 'Loading devices...', 'Almost ready...']
...
const statsCards = ref([
  { title: 'Honeywell Devices', value: '12', trend: 3, icon: 'Cpu', bgColor: '#409eff', key: 'devices', subTitle: 'Online: 10' },
  { title: 'Data Points', value: '245', trend: 8, icon: 'DataAnalysis', bgColor: '#67c23a', key: 'points', subTitle: 'Active: 238' },
  { title: 'Scan Rate', value: '856/s', trend: 5, icon: 'TrendCharts', bgColor: '#e6a23c', key: 'rate', subTitle: 'Peak: 1.2K/s' },
  { title: 'Alarms', value: '3', trend: -1, icon: 'Warning', bgColor: '#f56c6c', key: 'alarms', subTitle: 'Active: 1' }
])

const devices = ref([
  { id: 1, name: 'Plant_Controller_C300', deviceType: 'Controller', model: 'C300', ipAddress: '192.168.1.100', port: 502, protocol: 'Modbus TCP', unitId: 1, pointCount: 56, status: 'Online', lastSeen: '2024-01-20 10:30:00' },
  { id: 2, name: 'HVAC_Controller_HC900', deviceType: 'Controller', model: 'HC900', ipAddress: '192.168.1.101', port: 47808, protocol: 'BACnet IP', unitId: 1, pointCount: 42, status: 'Online', lastSeen: '2024-01-20 10:28:00' },
  { id: 3, name: 'Thermostat_Lyric_T6', deviceType: 'Thermostat', model: 'Lyric T6', ipAddress: '192.168.1.102', port: 80, protocol: 'MQTT', unitId: 1, pointCount: 12, status: 'Online', lastSeen: '2024-01-20 10:32:00' },
  { id: 4, name: 'Gateway_WEB_9000', deviceType: 'Gateway', model: 'WEB-9000', ipAddress: '192.168.1.103', port: 80, protocol: 'BACnet IP', unitId: 1, pointCount: 78, status: 'Online', lastSeen: '2024-01-20 10:29:00' },
  { id: 5, name: 'Temperature_Sensor', deviceType: 'Sensor', model: 'Rondostat', ipAddress: '192.168.1.104', port: 502, protocol: 'Modbus TCP', unitId: 2, pointCount: 8, status: 'Warning', lastSeen: '2024-01-20 10:25:00' }
])

const pointsMap = ref<Record<number, any[]>>({
  1: [
    { id: 1, address: '40001', name: 'Temperature', pointType: 'Analog Input', dataType: 'Float', unit: '°C', currentValue: 22.5, scaleFactor: 1, minRange: -10, maxRange: 50, description: 'Process temperature' },
    { id: 2, address: '40003', name: 'Pressure', pointType: 'Analog Input', dataType: 'Float', unit: 'kPa', currentValue: 101.3, scaleFactor: 1, minRange: 0, maxRange: 200, description: 'Process pressure' },
    { id: 3, address: '10001', name: 'Fan Status', pointType: 'Digital Input', dataType: 'Bool', unit: '', currentValue: true, scaleFactor: 1, description: 'Fan running status' }
  ],
  2: [
    { id: 4, address: 'AI-1', name: 'Room Temperature', pointType: 'Analog Input', dataType: 'Float', unit: '°C', currentValue: 23.0, scaleFactor: 1, minRange: 0, maxRange: 40, description: 'Room temperature sensor' }
  ]
})
...
const isWritable = (pointType: string) => {
  return pointType === 'Analog Output' || pointType === 'Digital Output' || pointType === 'Binary Output'
}

const handleCardClick = (stat: any) => ElMessage.info(`Viewing ${stat.title} details`)
const handleExport = () => ElMessage.success('Exporting Honeywell configuration...')
...
const openAddDeviceDialog = () => {
  ...
  Object.assign(deviceForm, {
    id: null, name: '', deviceType: 'Controller', model: 'C300', ipAddress: '', port: 502,
...
const testDeviceConnection = () => {
  ...
  setTimeout(() => {
    ...
    testResult.message = 'Successfully connected to Honeywell device'
    testResult.details = {
...
const openAddPointDialog = () => {
  ...
  Object.assign(pointForm, {
    id: null, address: '', name: '', pointType: 'Analog Input', dataType: 'Float', unit: '',
...
const readPoint = (point: any) => {
  ...
  setTimeout(() => {
    const newValue = point.dataType === 'Float' ? (Math.random() * 100).toFixed(2) : Math.random() > 0.5
...
const readAllPoints = () => {
  ...
  setTimeout(() => {
    points.value.forEach(point => {
      point.currentValue = point.dataType === 'Float' ? (Math.random() * 100).toFixed(2) : Math.random() > 0.5
    })
