<template>
    ...
    <div class="loading-overlay">
      <div class="loading-content">
        ...
        <div class="loading-text">
          <span class="loading-title">Water Monitoring</span>
        ...
        <div class="loading-tip">Water Consumption & Efficiency Analytics</div>
  ...
  <!-- Main Content -->
  <div v-else class="water-monitoring-page">
    <!-- Header -->
    <div class="page-header">
      <div>
        ...
        <div class="page-subtitle">Real-time water consumption tracking, efficiency analysis, and conservation insights</div>
      </div>
    ...
    <div class="metrics-row">
      ...
      <div class="metric-card">
        ...
        <div class="metric-value">{{ metrics.waterEfficiency }}<span class="metric-unit">%</span></div>
        <el-progress :percentage="metrics.waterEfficiency" :stroke-width="8" :color="metrics.waterEfficiency > 85 ? '#22c55e' : (metrics.waterEfficiency > 70 ? '#f59e0b' : '#ef4444')" />
      </div>
      <div class="metric-card">
        <div class="metric-title">Water Intensity</div>
        <div class="metric-value">{{ metrics.waterIntensity }}<span class="metric-unit">L/m²</span></div>
      ...
      <div class="metric-card">
        <div class="metric-title">Leak Detection</div>
        <div class="metric-value">{{ metrics.leakAlerts }}</div>
        <div class="metric-sub">Active alerts</div>
        <div class="metric-target" :class="metrics.leakAlerts > 0 ? 'metric-bad' : 'metric-good'">
          {{ metrics.leakAlerts > 0 ? 'Action required' : 'No leaks detected' }}
        </div>
      </div>
    </div>
    ...
    <div class="charts-row">
      <div class="chart-card large">
        <div class="chart-header">
          <span class="chart-title">Real-time Flow Rate</span>
    ...
    <div class="charts-row">
      <div class="chart-card">
        <div class="chart-header">
          ...
          <span class="chart-subtitle">Water usage distribution</span>
        </div>
    ...
    <div class="charts-row">
      ...
      <div class="chart-card">
        <div class="chart-header">
          <span class="chart-title">Water Balance</span>
          <span class="chart-subtitle">Inflow vs Consumption</span>
        </div>
    ...
    <div class="table-container">
      <div class="table-header">
        <span class="table-title">Water Meter Readings</span>
      ...
      <el-table :data="paginatedMeters" stripe style="width: 100%" v-loading="tableLoading">
        ...
        <el-table-column prop="leakDetected" label="Leak Status" width="120">
          <template #default="{ row }">
            <el-tag :type="row.leakDetected ? 'danger' : 'success'" size="small">
    ...
    <el-dialog v-model="detailDialogVisible" :title="selectedMeter?.name" width="1000px" class="meter-dialog">
      <div v-if="selectedMeter" class="meter-detail">
        ...
        <div class="detail-header-stats">
          ...
          <div class="detail-stat">
            <div class="detail-stat-value">{{ selectedMeter.waterEfficiency }}<span class="stat-unit">%</span></div>
            <div class="detail-stat-label">Water Efficiency</div>
          </div>
        </div>
        ...
        <el-descriptions :column="3" border>
          ...
          <el-descriptions-item label="Leak Detected">
            <el-tag :type="selectedMeter.leakDetected ? 'danger' : 'success'" size="small">
              {{ selectedMeter.leakDetected ? 'Yes' : 'No' }}
            </el-tag>
        ...
        <div class="detail-section">
          <div class="section-title">Hourly Flow Rate Trend</div>
...
import {
   DataLine, Calendar, Money, Download, Refresh,
  RefreshLeft
...
const loadingMessage = ref('Loading water monitoring data...')
...
// ==================== Mock Data ====================
const zones = ['Data Center A', 'Data Center B', 'Cooling Tower', 'Chiller Plant', 'Landscape', 'Restrooms', 'Kitchen', 'Laboratory']
const metersList = ['Main Meter', 'Cooling Tower Makeup', 'Chiller Supply', 'Chiller Return', 'Landscape Irrigation', 'Domestic Water']

const generateMeterData = (): WaterMeter[] => {
  const meters: WaterMeter[] = []

  for (let i = 1; i <= 20; i++) {
    const zone = zones[Math.floor(Math.random() * zones.length)]
    const capacity = [10, 20, 30, 50, 100, 200][Math.floor(Math.random() * 6)]
    const currentFlow = parseFloat((capacity * (0.2 + Math.random() * 0.6)).toFixed(1))
    const todayConsumption = Math.round(currentFlow * 18 + Math.random() * 100)
    const monthlyConsumption = Math.round(todayConsumption * 30 + Math.random() * 3000)
    const yearlyConsumption = monthlyConsumption * 12
    const waterEfficiency = Math.min(100, Math.max(50, 80 + Math.random() * 15))
    const leakDetected = Math.random() > 0.85

    let status = 'Normal'
    if (leakDetected) status = 'Critical'
    else if (currentFlow > capacity * 0.8) status = 'Warning'
    ...
    if (leakDetected) {
      alerts.push({
        ...
        type: 'Leak Detected',
        description: `Abnormal flow pattern detected - possible leak in ${zone}`,
        severity: 'High'
      })
    }
    if (waterEfficiency < 70) {
      alerts.push({
        ...
        type: 'Low Efficiency',
        description: `Water efficiency dropped to ${waterEfficiency}%, below target`,
        severity: 'Medium'
      })
    }

    meters.push({
      id: `WTR-${String(i).padStart(4, '0')}`,
      name: metersList[(i - 1) % metersList.length] + (Math.floor((i - 1) / metersList.length) + 1),
      zone: zone,
      location: `${zone} - ${['Main Line', 'Branch 1', 'Branch 2', 'Sub-meter'][Math.floor(Math.random() * 4)]}`,
      type: ['Ultrasonic Meter', 'Electromagnetic Meter', 'Mechanical Meter'][Math.floor(Math.random() * 3)],
      manufacturer: ['Siemens', 'Badger Meter', 'Kamstrup', 'Itron'][Math.floor(Math.random() * 4)],
...
const meters = ref<WaterMeter[]>(generateMeterData())
...
const selectedMeter = ref<WaterMeter | null>(null)
...
const stats = computed(() => {
  ...
  const estimatedCost = Math.round(monthlyConsumption * 0.5 / 1000) // $0.5 per m³
...
const metrics = computed(() => {
  ...
  const peakTime = '10:30'
  const capacity = meters.value.reduce((sum, m) => sum + m.capacity, 0)
  const avgWaterEfficiency = Math.round(meters.value.reduce((sum, m) => sum + m.waterEfficiency, 0) / meters.value.length)
  const waterIntensity = parseFloat((stats.value.monthlyConsumption / 5000).toFixed(1))
  const leakAlerts = meters.value.filter(m => m.leakDetected).length

  return {
    peakFlow: peakFlow,
    peakTime: peakTime,
    capacity: Math.round(capacity),
    waterEfficiency: avgWaterEfficiency,
...
const initRealtimeChart = () => {
  ...
  const flowData = labels.map(() => parseFloat((50 + Math.random() * 80).toFixed(1)))
...
const initWeeklyChart = () => {
  ...
  const consumption = [185, 192, 178, 195, 210, 165, 142]
...
const initMonthlyChart = () => {
  ...
  const consumption = [5200, 4900, 5100, 5300, 5600, 5800, 6200, 6400, 6100, 5700, 5400, 5100]
...
const initBalanceChart = () => {
  ...
  const categories = ['Cooling Tower', 'Chiller', 'Landscape', 'Domestic', 'Other']
  const inflow = [350, 280, 120, 180, 70]
  const consumption = [320, 260, 115, 175, 65]

  balanceChart = echarts.init(balanceChartEl.value)
  balanceChart.setOption({
    tooltip: { trigger: 'axis', axisPointer: { type: 'shadow' } },
    legend: { data: ['Inflow', 'Consumption'], bottom: 0 },
    grid: { top: 30, left: 50, right: 20, bottom: 40 },
    xAxis: { type: 'category', data: categories, axisLabel: { rotate: 30 } },
    yAxis: { type: 'value', name: 'Volume (m³/day)' },
    series: [
      { name: 'Inflow', type: 'bar', data: inflow, itemStyle: { color: '#0ea5e9', borderRadius: [4, 4, 0, 0] }, label: { show: true, position: 'top', formatter: '{c} m³' } },
      { name: 'Consumption', type: 'bar', data: consumption, itemStyle: { color: '#22c55e', borderRadius: [4, 4, 0, 0] }, label: { show: true, position: 'top', formatter: '{c} m³' } }
    ]
  })
}
...
// ==================== Actions ====================
const viewMeterDetail = (meter: WaterMeter) => {
...
const viewAllMeters = () => {
  ElMessage.info('Viewing all water meters')
}

const exportMeterReport = (meter: WaterMeter | null) => {
  if (!meter) return
...
const exportData = () => {
  ElMessage.success('Exporting water monitoring data...')
  setTimeout(() => {
...
const handleResize = () => {
  ...
  resizeTimer = setTimeout(() => {
    const charts = [realtimeChart, zoneChart, weeklyChart, monthlyChart, balanceChart, meterDetailChart]
