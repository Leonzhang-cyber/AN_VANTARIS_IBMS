...
const initHeatmapChart = () => {
  ...
  // Generate heatmap data: [decisionType, priority, approvalLevels]
  const decisionTypes = ['Fault Decision', 'Maintenance Decision', 'ESG Decision', 'Energy Decision', 'AI Recommendation']
  const priorities = ['Critical', 'High', 'Medium', 'Low']
  const heatmapData: [number, number, number][] = []

  // Mock heatmap data - approval levels required for each combination
  const levelMatrix = [
...
const submitForm = async () => {
  ...
  await formRef.value.validate((valid: boolean) => {
    if (valid) {
      // Map approver IDs to full approver objects
