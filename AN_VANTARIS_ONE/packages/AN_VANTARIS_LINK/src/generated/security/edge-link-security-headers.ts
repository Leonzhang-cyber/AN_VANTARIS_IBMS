...
export function computeWireEventSignature(
  ...
  return createHmac('sha256', signingSecret).update(serializeWireEventSignInput(payload)).digest('hex');
}
...
export function verifyWireEventSignature(event: WireTransportEvent, signingSecret: string): boolean {
  const expected = computeWireEventSignature(buildWireEventSignPayload(event), signingSecret);
...
export function extractLinkIngressHeaders(
  ...
  return {
    machineId: headers['x-vantaris-machine-id'] ?? headers.machineId ?? '',
    credentialRef: headers.credentialRef ?? '',
    timestamp: headers['x-vantaris-signed-at'] ?? headers.timestamp ?? '',
    signature,
  };
}
