import type {
  SnmpNormalizedTarget,
  SnmpReadOnlyPolicy,
  SnmpTargetRiskEvaluation,
  SnmpTargetValidation,
...
export function normalizeSnmpTarget(target: string, port: number | undefined, policy: SnmpReadOnlyPolicy): SnmpNormalizedTarget | null {
  if (!target || typeof target !== 'string') return null;
  if (target.includes('@')) return null;

  let hostname = target.trim();
  let resolvedPort = port ?? policy.defaultPort;
...
export function evaluateSnmpTargetRisk(
  normalized: SnmpNormalizedTarget,
  ...
  if (denied.has(normalized.hostname) || normalized.hostname === 'localhost') {
    ...
    errors.push('SNMP_TARGET_LOCALHOST_REJECTED');
  }
  if (normalized.hostname === 'metadata.google.internal') {
    risks.push('METADATA');
    errors.push('SNMP_TARGET_METADATA_REJECTED');
  }
  ...
  if (parsedIp) {
    ...
    for (const category of categories) {
      risks.push(category);
      errors.push(`SNMP_TARGET_${category}_REJECTED`);
    }
  } else {
    ...
    if (!allowed.includes(normalized.hostname)) {
      ...
      errors.push('SNMP_TARGET_NOT_ALLOWLISTED');
    }
  }
...
export function validateSnmpTarget(
  target: string,
  port: number | undefined,
  policy: SnmpReadOnlyPolicy,
): SnmpTargetValidation {
  if (target.includes('@')) {
    return { ok: false, errors: ['SNMP_TARGET_MALFORMED'] };
  }
  const normalized = normalizeSnmpTarget(target, port, policy);
  if (!normalized) return { ok: false, errors: ['SNMP_TARGET_MALFORMED'] };
  const risk = evaluateSnmpTargetRisk(normalized, policy);
  return { ok: risk.ok, target: normalized, errors: risk.errors };
}

export function validateSnmpCredentialModel(
  input: {
  ...
  const errors: string[] = [];
  if (input.community) errors.push('PLAINTEXT_COMMUNITY_PROHIBITED');
  if (input.authPassword || input.privPassword || input.username) errors.push('PLAINTEXT_SNMP_SECRET_PROHIBITED');
  if (policy.plaintextCommunityAllowed) errors.push('PLAINTEXT_COMMUNITY_PROHIBITED');
  if (policy.plaintextAuthSecretAllowed || policy.plaintextPrivacySecretAllowed) {
    errors.push('PLAINTEXT_SNMP_SECRET_PROHIBITED');
  }
  if (policy.credentialMode !== 'REFERENCE_ONLY') errors.push('PLAINTEXT_COMMUNITY_PROHIBITED');
  if (input.credentialRef && !/^secret:\/\/edge\/snmp\/.+/.test(input.credentialRef)) {
    errors.push('PLAINTEXT_SNMP_SECRET_PROHIBITED');
  }
