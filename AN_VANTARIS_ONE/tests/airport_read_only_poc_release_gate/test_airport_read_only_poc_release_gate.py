"""Tests for ONE-AIRPORT-A9-01 final read-only POC release gate."""
from __future__ import annotations

import json
import unittest

from airport_read_only_poc_release_gate.validation import EXPECTED_SUMMARY, validate_airport_read_only_poc_release_gate
from industry_profiles.airport.airport_read_only_poc_release_gate import (
    IMPLEMENTATION_STATUS,
    READINESS_OUTCOME,
    build_airport_read_only_poc_release_gate,
)


class AirportReadOnlyPocReleaseGateTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.gate = build_airport_read_only_poc_release_gate()
    ...
    def test_contract_and_readiness_freeze(self) -> None:
        self.assertEqual("airport-read-only-poc-release-gate.v1", self.gate["contractVersion"])
    ...
    def test_release_decision(self) -> None:
        ...
        self.assertEqual("READ_ONLY_POC_RELEASE_CANDIDATE_PASS", decision["decisionState"])
    ...
    def test_validator_accepts_gate(self) -> None:
        validate_airport_read_only_poc_release_gate(self.gate)
