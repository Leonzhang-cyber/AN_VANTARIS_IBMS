"""Airport International GA release aggregation and final gate."""
...
from airport_international_ga_release_gate.enums import GateStatus, ReadinessState, ReleaseDecisionState, Severity
from airport_international_ga_release_gate.models import (
    build_airport_international_ga_release_gate as build_airport_international_ga_release_gate_model,
...
from airport_international_ga_release_gate.validation import validate_airport_international_ga_release_gate
from source_system_registry.digest import sha256_digest

AUTHORITY = "ONE-AIRPORT-GA-01"
PROFILE_ID = "airport-international-ga-release-gate-profile-v1"
IMPLEMENTATION_STATUS = "AIRPORT_INTERNATIONAL_GA_RELEASE_TERMINOLOGY_ALIGNMENT_COMPLETE"
READINESS_OUTCOME = "AIRPORT_INTERNATIONAL_GA_READINESS_CANDIDATE_PASS"
...
def _release_gates() -> list[dict[str, Any]]:
    specs = [
        ...
        ("G20_INTERNATIONAL_GA_READINESS_DECISION", "International GA readiness decision", ["INTERNATIONAL_GA_READINESS_PASS"]),
    ]
    return [build_release_gate_result(gate_id=gate_id, gate_name=name, status=GateStatus.PASS.value, severity=Severity.INFO.value if gate_id != "G20_INTERNATIONAL_GA_READINESS_DECISION" else Severity.LOW.value, blocking=True, reason_codes=reasons) for gate_id, name, reasons in specs]


def _decision() -> dict[str, Any]:
    return build_release_decision(
        decision_state=ReleaseDecisionState.INTERNATIONAL_GA_READINESS_PASS.value,
        international_ga_readiness_allowed=True,
        ...
        decision_reason="Airport International GA-ready read-only foundation is complete; push, tag, runtime activation, and production activation remain disabled until explicitly authorized.",
    )
...
def build_airport_international_ga_release_gate() -> dict[str, Any]:
    summary = _summary()
    gate = build_airport_international_ga_release_gate_model(
    ...
    validate_airport_international_ga_release_gate(gate)
    return gate
